...
TABLE statements in `backend/database/init_database.py` — the canonical schema per
`docs/plans/active/production-nightly-deploys/production-nightly-deploys.md` — and writes them to a generated manifest that
the exporter reads.

Usage:
  python src/tools/generate_export_schema.py --write     # regenerate the manifest
  python src/tools/generate_export_schema.py --check     # fail if the manifest is stale
  docker compose exec backend python src/tools/generate_export_schema.py --check-db  # compare against the live database
"""
...
INIT_SCRIPT = ROOT / "backend" / "database" / "init_database.py"
...
def build_manifest() -> dict:
    ...
    return {
        "_generated_by": "src/tools/generate_export_schema.py",
        "_source": "backend/database/init_database.py",
...
def load_manifest() -> dict[str, list[str]]:
    ...
    if not MANIFEST_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run: python src/tools/generate_export_schema.py --write"
        )
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                f"[STALE] {MANIFEST_PATH.relative_to(ROOT)} does not match backend/database/init_database.py.\n"
                "        Run: python src/tools/generate_export_schema.py --write",
                file=sys.stderr,
            )
            return 1
