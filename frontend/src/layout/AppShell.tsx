    ...
    <AppShellRowSlotsContext.Provider value={{ identitySlot, tabsSlot }}>
      <div className="app-shell">
        ...
        <div className="app-body">
          <nav
            className={`app-nav ${collapsed ? "app-nav--collapsed" : ""} ${isEncounterRunner ? "app-nav--play" : ""}`}
          >
            <Link to="/" className="app-brand">
              <MapIcon size={22} aria-hidden="true" />
              <span className={collapsed || isEncounterRunner ? "visually-hidden" : undefined}>
                D&D Kids Resources
              </span>
            </Link>
            <IconButton
            ...
            {navSections.map((section) => (
              <div className="app-nav-section" key={section.label}>
                <h2 className={collapsed || isEncounterRunner ? "visually-hidden" : undefined}>
                  {section.label}
                </h2>
                <ul>
                  {section.links.map((link) => (
                    <li key={link.to}>
                      ...
                      >
                        ...
                        <span
                          className={collapsed || isEncounterRunner ? "visually-hidden" : undefined}
                        >
                          {link.label}
                        </span>
                      </NavLink>
