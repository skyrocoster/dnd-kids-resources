  ...
  for (let hue = hueLo; hue <= hueHi; hue += HUE_STEP) {
    for (const chroma of CHROMA_SEARCH) {
      for (const tone of TONE_SEARCH) {
        ...
        const allBgOk = BACKGROUND_LSTARS.every(
          (bgL) => Contrast.ratioOfTones(actualTone, bgL) >= CONTRAST_FLOOR,
        );
        if (!allBgOk) continue;
 ...
 * @param {string[]} _reservedHexes — hex strings the palette must stay clear of
 * @param {number[]} _bgLstars      — background L* values for contrast check
 * @returns {object} family key → {disc, glyph, hue, chroma, tone}
 */
export function solvePalette(
  familySpec = FAMILY_SPEC,
  ...
  for (const key of pickOrder) {
    ...
    for (const cand of allCandidates[key]) {
      ...
      if (Object.keys(picked).length === 0) {
        ...
        const toneClashPenalty = Object.values(picked).some((p) => Math.abs(cand.tone - p.tone) < 5)
          ? 8
          : 0;
...
export function formatTokenBlock(result, familySpec = FAMILY_SPEC) {
  ...
  for (const key of keys) {
    ...
    lines.push(
      `   hue ${info.hue.toFixed(1)}, chroma ${info.chroma.toFixed(1)}, tone ${info.tone.toFixed(1)}. */`,
    );
...
if (
  scriptPath.endsWith("/derive-kid-palette.mjs") ||
  scriptPath.endsWith("\\derive-kid-palette.mjs")
) {
  main();
}
