...
describe('generated API client facade', () => {
  ...
  it('preserves opaque nested JSON in generated response data and request bodies', async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValueOnce(jsonResponse(layout)).mockResolvedValueOnce(jsonResponse(layout))
    vi.stubGlobal('fetch', fetchMock)
  ...
  it('routes monster create, update, and delete through the generated operations', async () => {
    ...
    const [createRequest, updateRequest, deleteRequest] = fetchMock.mock.calls.map(([request]) => request as Request)
  ...
  it('converts structured backend errors to status-aware ApiError values', async () => {
    ...
    await expect(getAbilities()).rejects.toMatchObject(new ApiError(404, 'Spell not found', 'spell_not_found'))
  })
