...
export function ScrollArea({
  ...
  return (
    <BaseScrollArea.Root
      {...rest}
      className={["cp-scroll-area", className].filter(Boolean).join(" ")}
    >
      <BaseScrollArea.Viewport
        className={["cp-scroll-viewport", viewportClassName].filter(Boolean).join(" ")}
      >
        <BaseScrollArea.Content
          className={["cp-scroll-content", contentClassName].filter(Boolean).join(" ")}
        >
          {children}
        </BaseScrollArea.Content>
      </BaseScrollArea.Viewport>
      {orientation !== "horizontal" && (
        <BaseScrollArea.Scrollbar className={scrollbarClasses} orientation="vertical">
          <BaseScrollArea.Thumb className="cp-scroll-thumb" />
