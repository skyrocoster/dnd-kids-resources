...
export type FormProps<T extends Record<string, unknown> = Record<string, unknown>> = Omit<
  BaseFormProps<T>,
  "className"
> & { className?: string; layout?: FormLayout };
export function Form<T extends Record<string, unknown> = Record<string, unknown>>({
  ...
  return (
    <BaseForm<T>
      {...rest}
      className={["form-advanced", className].filter(Boolean).join(" ")}
      data-layout={layout}
    />
  );
}
