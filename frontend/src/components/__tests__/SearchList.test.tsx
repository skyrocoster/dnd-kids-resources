...
describe("SearchList", () => {
  ...
  it("filters items by search query", async () => {
    ...
    render(
      <SearchList items={items} getId={(i) => i.id} getLabel={(i) => i.name} onSelect={() => {}} />,
    );
  ...
  it("calls onSelect with the clicked item", async () => {
    ...
    render(
      <SearchList items={items} getId={(i) => i.id} getLabel={(i) => i.name} onSelect={onSelect} />,
    );
  ...
  it("marks the selected item as active", () => {
    ...
    expect(screen.getByRole("button", { name: /Magic Missile/ })).toHaveAttribute(
      "aria-current",
      "true",
    );
  });
