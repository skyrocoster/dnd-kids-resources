...
import { formatUtcDate, getUtcCalendarDay, normalizeToUtcMidnight, toUtcMidnight } from "../CalendarDateUtils";
...
describe("CalendarDate UTC helpers", () => {
  it("formats and normalizes a UTC calendar day", () => {
    ...
    expect(getUtcCalendarDay(toUtcMidnight({ year: 2025, month: 0, day: 15 }))).toEqual({ year: 2025, month: 0, day: 15 });
  });
});
...
describe("CalendarDate", () => {
  it("opens a calendar, selects a day as UTC midnight, and closes", async () => {
    ...
    const onChange = vi.fn((next: CalendarDateValue) => { value = next; });
