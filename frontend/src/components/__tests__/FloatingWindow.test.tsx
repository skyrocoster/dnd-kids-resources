...
describe("loadSize / saveSize round-trip", () => {
  ...
  it("returns DEFAULT_SIZE when nothing is stored", () => {
    ...
    render(
      <FloatingWindow title="Test" storageKey="fresh-key" onClose={() => {}}>
        content
      </FloatingWindow>,
    );
  ...
  it("persists and retrieves a clamped size", () => {
    render(
      <FloatingWindow title="Test" storageKey={storageKey} onClose={() => {}}>
        content
      </FloatingWindow>,
    );
...
describe("FloatingWindow resize", () => {
  ...
  it("renders a resize handle with accessible label", () => {
    render(
      <FloatingWindow title="Test" storageKey="k" onClose={() => {}}>
        content
      </FloatingWindow>,
    );
  ...
  it("minimizes and restores without changing the window geometry, and closes on request", () => {
    const onClose = vi.fn();
    render(
      <FloatingWindow title="Test" storageKey="minimize-test" onClose={onClose}>
        content
      </FloatingWindow>,
    );
  ...
  it("keyboard arrow keys resize the window and persist", () => {
    render(
      <FloatingWindow title="Test" storageKey="keyboard-test" onClose={() => {}}>
        content
      </FloatingWindow>,
    );
    const handle = screen.getByRole("separator", { name: /resize window/i });
  ...
  it("resize does not trigger when dragging the header", () => {
    render(
      <FloatingWindow title="Test" storageKey="no-drag" onClose={() => {}}>
        content
      </FloatingWindow>,
    );
    const windowEl = screen.getByRole("dialog");
    ...
    fireEvent(
      handle,
      new PointerEvent("pointerdown", { clientX: 400, clientY: 500, bubbles: true }),
    );
    ...
    fireEvent(
      handle,
      new PointerEvent("pointerdown", { clientX: 450, clientY: 560, bubbles: true }),
    );
