...
describe("glossaryTerms", () => {
  ...
  it("alias save resolves to saving throw definition", () => {
    ...
    if (nodes[1].type === "term") {
      ...
      expect(nodes[1].definition.definition).toBe("A roll to dodge, resist, or shrug off.");
    }
  });
  ...
  it("createGlossaryRegistry throws on duplicate key", () => {
    ...
    expect(() => createGlossaryRegistry(defs)).toThrow("Duplicate glossary term: test");
  });
  ...
  it("matches all eight combat action terms in running text", () => {
    ...
    const termTexts = nodes.filter((n) => n.type === "term").map((n) => n.text.toLowerCase());
