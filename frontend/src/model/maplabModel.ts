...
export function doorWallSegment(
  edge: WallEdge,
  cellSize: number,
...
export function markersAtCell(
  layout: MapLayout,
  z: number,
  cell: MapCell,
): Array<{ type: "stair" | "portal" | "prop"; id: number }> {
  const [cx, cy] = cell;
  ...
  const props = layout.props
    .filter(
      (prop) =>
        prop.side === undefined && prop.z === z && prop.cell[0] === cx && prop.cell[1] === cy,
    )
...
export function ghostFloorZ(layout: MapLayout, activeZ: number): number | null {
  const lowerZs = layout.rooms.map((room) => room.z).filter((z) => z < activeZ);
...
export function npcIdsFromMarkersInRoom(layout: MapLayout, room: MapRoom): number[] {
  ...
  for (const prop of floorProps) {
    if (
      prop.kind === "npc" &&
      prop.npc_id !== null &&
      prop.npc_id !== undefined &&
      roomCells.has(cellKey(prop.cell))
    ) {
...
export function getNpcUnion(
  explicitNpcIds: number[] | null | undefined,
  room: MapRoom | null | undefined,
...
export function effectiveFixtureState(
  ...
  return {
    ...
    obstacles: {
      concealment: {
        armed:
          session?.obstacles?.concealment?.armed ??
          authored.obstacles?.concealment?.armed ??
          defaults.obstacles.concealment.armed,
      },
      lock: {
        armed:
          session?.obstacles?.lock?.armed ??
          authored.obstacles?.lock?.armed ??
          defaults.obstacles.lock.armed,
        shown:
          session?.obstacles?.lock?.shown ??
          authored.obstacles?.lock?.shown ??
          defaults.obstacles.lock.shown,
      },
      trap: {
        armed:
          session?.obstacles?.trap?.armed ??
          authored.obstacles?.trap?.armed ??
          defaults.obstacles.trap.armed,
        shown:
          session?.obstacles?.trap?.shown ??
          authored.obstacles?.trap?.shown ??
          defaults.obstacles.trap.shown,
      },
    },
  };
}
...
export function sharedWallSegments(roomA: MapRoom, roomB: MapRoom, _doors: MapDoor[]): WallEdge[] {
  ...
  return roomWallSegments(roomA).filter((edge) =>
    bCells.has(cellKey(neighborCell(edge.cell, edge.side))),
  );
}

export function stairDirection(
  stair: MapStair,
  fromZ: number = stair.from.z,
): "up" | "down" | "level" {
...
export function normalizeLayout(layout: MapLayout): MapLayout {
  ...
  const meta = {
    ...(typeof layout.meta.padding === "number"
      ? {
          ...layout.meta,
          padding: {
            top: layout.meta.padding,
            right: layout.meta.padding,
            bottom: layout.meta.padding,
            left: layout.meta.padding,
          },
        }
      : layout.meta),
    nextDoorId:
      layout.meta.nextDoorId ?? Math.max(0, ...(layout.doors ?? []).map((d) => d.door_id)) + 1,
    nextPropId: layout.meta.nextPropId ?? Math.max(0, ...props.map((p) => p.prop_id)) + 1,
    nextStairId:
      layout.meta.nextStairId ?? Math.max(0, ...(layout.stairs ?? []).map((s) => s.stair_id)) + 1,
  ...
  return {
    ...
    doors: layout.doors.map((door) =>
      door.z !== undefined ? door : { ...door, z: inferZ(door.cell) },
    ),
    portals,
    features,
  };
