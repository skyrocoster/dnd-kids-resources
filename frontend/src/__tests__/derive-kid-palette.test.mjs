...
import { Hct, Contrast, argbFromHex, lstarFromArgb, labFromArgb, yFromLstar } from '../../node_modules/@material/material-color-utilities/index.js'
...
describe('kid palette solver', () => {
  ...
  it('glyph colour is either black or white', () => {
    ...
    for (const key of FAMILY_KEYS) {
      ...
      expect(glyph === '#000000' || glyph === '#FFFFFF',
        `"${key}" glyph is ${glyph}, expected #000000 or #FFFFFF`).toBe(true)
    }
  })
  ...
  it('each glyph has ≥4.5:1 contrast on its disc', () => {
    ...
    for (const key of FAMILY_KEYS) {
      ...
      expect(cr, `"${key}" glyph contrast on disc is ${cr.toFixed(2)}:1`).toBeGreaterThanOrEqual(4.5)
    }
  })
  ...
  it('all families pass contrast floor (≥4.5:1) against both backgrounds', () => {
    ...
    for (const key of FAMILY_KEYS) {
      ...
      for (const bgL of BACKGROUND_LSTARS) {
        ...
        expect(cr,
          `"${key}" (L* ${discLstar.toFixed(1)}) vs background L* ${bgL}: contrast ${cr.toFixed(2)}:1`).toBeGreaterThanOrEqual(4.5)
      }
    }
  })
  ...
  it('ΔE2000 between any two family colours is ≥15', () => {
    ...
    for (let i = 0; i < FAMILY_KEYS.length; i++) {
      for (let j = i + 1; j < FAMILY_KEYS.length; j++) {
        ...
        expect(de,
          `ΔE2000(${FAMILY_KEYS[i]}, ${FAMILY_KEYS[j]}) = ${de.toFixed(1)}`).toBeGreaterThanOrEqual(15)
      }
    }
  })
  ...
  it('ΔE2000 from each family colour to each reserved colour is ≥10', () => {
    ...
    for (const key of FAMILY_KEYS) {
      ...
      for (let ri = 0; ri < RESERVED_HEXES.length; ri++) {
        ...
        expect(de,
          `ΔE2000(${key}, reserved[${ri}]) = ${de.toFixed(1)}`).toBeGreaterThanOrEqual(10)
      }
    }
  })
  ...
  it('formatTokenBlock emits a valid CSS block with all families', () => {
    ...
    for (const key of FAMILY_KEYS) {
      ...
      expect(block.includes(`--kid-on-${key}:`), `missing --kid-on-${key} token in output`).toBe(true)
    }

    // Proper CSS property lines
    const lines = block.split('\n').filter(l => l.startsWith('--kid-'))
    expect(lines.length, `expected ${FAMILY_KEYS.length * 2} token lines`).toBe(FAMILY_KEYS.length * 2)
    ...
    for (const key of FAMILY_KEYS) {
      ...
      expect(block.includes(`hue ${info.hue.toFixed(1)}`),
        `missing hue detail for "${key}"`).toBe(true)
      expect(block.includes(`chroma ${info.chroma.toFixed(1)}`),
        `missing chroma detail for "${key}"`).toBe(true)
      expect(block.includes(`tone ${info.tone.toFixed(1)}`),
        `missing tone detail for "${key}"`).toBe(true)
    }
  })
})
