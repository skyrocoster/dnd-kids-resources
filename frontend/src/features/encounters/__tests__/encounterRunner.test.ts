...
describe("setStatus / rename", () => {
  it("updates status for the targeted combatant only", () => {
    const state = hydrated();
    const clientId = state.combatants[0].clientId;
    const next = encounterRunnerReducer(state, {
      type: "setStatus",
      clientId,
      status: "unconscious",
    });
...
describe("setConditions", () => {
  ...
  it("clears conditions back to an empty array", () => {
    const state = hydrated();
    const clientId = state.combatants[0].clientId;
    const withConditions = encounterRunnerReducer(state, {
      type: "setConditions",
      clientId,
      conditions: ["prone"],
    });
    const cleared = encounterRunnerReducer(withConditions, {
      type: "setConditions",
      clientId,
      conditions: [],
    });
...
describe("duplicate", () => {
  it("inserts a full-HP copy directly after the source, keeping name/ac and a new clientId", () => {
    ...
    const damaged = encounterRunnerReducer(state, {
      type: "adjustHp",
...
describe("combatantsToCreatures", () => {
  it("strips clientId, preserves order and conditions, and derives active_index", () => {
    const state = hydrated({
      ...baseEncounter,
      active_index: 1,
      creatures: baseEncounter.creatures!.map((c, i) =>
        i === 0 ? { ...c, conditions: ["prone"] } : c,
      ),
    });
  ...
  it("kind field survives combatantsToCreatures -> hydrate round-trip", () => {
    const state = encounterRunnerReducer(hydrated(), {
      type: "addPlayer",
      name: "Frodo",
      conditions: [],
    });
...
describe("combatantFromMonster", () => {
  it("assigns a fresh clientId each call", () => {
    const rat = monster({
      id: 1,
      name: "Rat",
      ac: { value: 11, note: null, alternatives: [] },
      hp: { average: 4, formula: null },
    });
