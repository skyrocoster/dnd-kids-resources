...
describe("EncounterEditor", () => {
  ...
  describe("C2: Condition checkboxes", () => {
    ...
    it("legacy/unknown conditions survive an edit", async () => {
      ...
      expect(
        await screen.findByRole("checkbox", { name: "stunned (legacy) (custom)" }),
      ).toBeChecked();
    });
  });
  ...
  describe("C4: Creature row collapse", () => {
    ...
    it("keeps expansion state independent for each creature row", async () => {
      ...
      expect(screen.getAllByRole("button", { name: "Unnamed creature" })[0]).toHaveAttribute(
        "aria-expanded",
        "false",
      );
      expect(screen.getAllByRole("button", { name: "Unnamed creature" })[1]).toHaveAttribute(
        "aria-expanded",
        "true",
      );
  ...
  describe("C5: Long creature list", () => {
    it("renders multiple creature rows", async () => {
      ...
      render(<EncounterEditor onClose={() => {}} onSaved={() => {}} />);
      expect(screen.getByRole("button", { name: "Add Creature" })).toHaveAttribute(
        "type",
        "button",
      );
      for (let i = 0; i < 5; i++) {
