...
export function EncounterEditor({ encounter, onClose, onSaved }: EncounterEditorProps) {
  ...
  const patch = (fields: Partial<EncounterFormState>) =>
  ...
  const handlePickMonster = (rowId: string, monsterId: string) => {
    ...
    const { hpAverage, ac } = monster
      ? deriveCreatureStats(monster)
      : { hpAverage: null, ac: null };
    updateRow(rowId, {
      monsterId,
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const saved = encounter
        ? await api.updateEncounter(encounter.id, payload)
        : await api.createEncounter(payload);
      setStatus({ message: "Encounter saved.", kind: "success" });
      onSaved(saved);
    } catch (error) {
      setStatus({
        message: error instanceof Error ? error.message : "Failed to save encounter.",
        kind: "error",
      });
      setSaving(false);
    }
    ...
    >
      ...
      <form id={formId} onSubmit={handleSubmit} className="encounter-editor-form">
        ...
        <section className="encounter-editor-section">
          ...
          {form.creatureRows.length === 0 && (
            <p className="encounter-editor-empty">No creatures added.</p>
          )}
