...
export function combatantsToCreatures(state: RunnerState): {
  creatures: EncounterCreature[];
  active_index: number | null;
} {
  const creatures: EncounterCreature[] = state.combatants.map(
    ({ clientId: _clientId, ...creature }) => creature,
  );
...
export function encounterRunnerReducer(state: RunnerState, action: RunnerAction): RunnerState {
  switch (action.type) {
    ...
    case "addPlayer":
      return {
        ...state,
        combatants: [...state.combatants, combatantFromPlayer(action.name, action.conditions)],
      };
    ...
    case "remove": {
      ...
      if (state.activeClientId === action.clientId) {
        activeClientId =
          combatants.length === 0
            ? null
            : combatants[Math.min(index, combatants.length - 1)].clientId;
      }
