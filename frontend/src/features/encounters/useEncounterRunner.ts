...
export function useEncounterRunner(encounterId: number): UseEncounterRunnerResult {
  ...
  const adjustHp = useCallback(
    (clientId: string, delta: number) => apply({ type: "adjustHp", clientId, delta }),
    [apply],
  );
  const setHp = useCallback(
    (clientId: string, hp: number) => apply({ type: "setHp", clientId, hp }),
    [apply],
  );
  const setStatus = useCallback(
    (clientId: string, status: string) => apply({ type: "setStatus", clientId, status }),
    [apply],
  );
  const setConditions = useCallback(
    (clientId: string, conditions: string[]) =>
      apply({ type: "setConditions", clientId, conditions }),
    [apply],
  );
  const rename = useCallback(
    (clientId: string, name: string) => apply({ type: "rename", clientId, name }),
    [apply],
  );
  const duplicate = useCallback(
    (clientId: string) => apply({ type: "duplicate", clientId }),
    [apply],
  );
  const addFromMonster = useCallback(
    (monster: Monster) => apply({ type: "addFromMonster", monster }),
    [apply],
  );
  ...
  const moveDown = useCallback(
    (clientId: string) => apply({ type: "moveDown", clientId }),
    [apply],
  );
  const setActive = useCallback(
    (clientId: string) => apply({ type: "setActive", clientId }),
    [apply],
  );
