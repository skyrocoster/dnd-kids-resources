...
describe("loot catalog pickers", () => {
  it("render catalog results and add selected entries", async () => {
    ...
    render(
      <>
        <AddItemPanel onAdd={addItem} onClose={() => {}} />
        <AddWeaponPanel onAdd={addWeapon} onClose={() => {}} />
      </>,
    );
  ...
  it("uses shared loading and error states instead of an empty catalog", async () => {
    ...
    render(
      <>
        <AddItemPanel onAdd={() => {}} onClose={() => {}} />
        <AddWeaponPanel onAdd={() => {}} onClose={() => {}} />
      </>,
    );
...
describe("LootBundleEditor", () => {
  it("adds an item, updates the live total, and saves its snapshot", async () => {
    ...
    const create = vi
      .spyOn(api, "createLootBundle")
      .mockResolvedValue({ id: 3, name: "Chest", gold: 12.5, contents: [] });
    const user = userEvent.setup();
    ...
    await waitFor(() =>
      expect(create).toHaveBeenCalledWith({
        ...
        gold: 12.5,
        contents: [
          expect.objectContaining({ kind: "item", name: "Ruby", value_gp: 50, quantity: 1 }),
        ],
      }),
    );
  });
...
describe("VW6 control conventions", () => {
  it("uses the workspace touch target and narrow breakpoint conventions", async () => {
    ...
    expect(readFileSync(resolve(root, "src/features/loot/AddCatalogPanel.css"), "utf-8")).toContain(
      "width: var(--control-height)",
    );
    expect(
      readFileSync(resolve(root, "src/features/loot/LootBundleEditor.css"), "utf-8"),
