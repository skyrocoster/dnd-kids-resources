...
function parseFeatureLines(text: string): { name: string; description: string; attack: null }[] {
  return text
    ...
    .map((line) => {
      ...
      return { name: line.slice(0, colon).trim(), description: line.slice(colon + 1).trim(), attack: null }
    })
}
...
export function playerToFormState(player: Player): PlayerFormState {
  ...
  return {
    ...
    bonusActionsText: (features?.bonus_actions || []).map((t) => `${t.name}: ${t.description || ''}`).join('\n'),
    ...
    legendaryActionsPerRound:
      features?.legendary_actions_per_round != null ? String(features.legendary_actions_per_round) : '',
    mythicActionsText: (features?.mythic_actions || []).map((t) => `${t.name}: ${t.description || ''}`).join('\n'),
    spellcastingText: (features?.spellcasting || [])
      .map((s) => {
        const desc = s.description ? `\n${s.description}` : ''
        const groups = s.groups.map((g) => `${g.label}: ${g.spells.map((sp) => sp.name).join(', ')}`).join('\n')
...
export function formStateToPlayerInput(form: PlayerFormState): PlayerInput {
  ...
  return {
    ...
    damage_resistances: textToDamageModifiers(form.damageResistances).map((m) => ({ ...m, conditional: false })),
    damage_immunities: textToDamageModifiers(form.damageImmunities).map((m) => ({ ...m, conditional: false })),
