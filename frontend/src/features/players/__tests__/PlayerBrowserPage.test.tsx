...
describe('PlayerBrowserPage', () => {
  ...
  it('shows the destructive delete confirmation copy', async () => {
    ...
    expect(
      screen.getByText('Delete "Lark"? Spell and weapon assignments will be removed. Catalog records will remain. This cannot be undone.'),
    ).toBeInTheDocument()
  })
  ...
  it('stages a spell change through Manage Spells and commits it via one atomic Save', async () => {
    vi.spyOn(api, 'listPlayers').mockResolvedValue(players)
    const replacePlayerSpells = vi.spyOn(api, 'replacePlayerSpells').mockResolvedValue([spellFireball])
    const user = userEvent.setup()
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Manage Spells' })).not.toBeInTheDocument())
  })
