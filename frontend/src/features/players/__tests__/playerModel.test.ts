...
describe('hasCombatStats', () => {
  it('is true when any of AC/HP/speed is present', () => {
    expect(hasCombatStats(makePlayer({ ac: { value: 16, note: null, alternatives: [] } }))).toBe(true)
    expect(hasCombatStats(makePlayer({ hp: { average: 22, formula: null } }))).toBe(true)
    expect(hasCombatStats(makePlayer({ speed: [{ mode: 'walk', feet: 30, note: null, hover: false }] }))).toBe(true)
  })
...
describe('hasStatblock', () => {
  ...
  it('returns true when only ac is populated', () => {
    expect(hasStatblock(makePlayer({ ac: { value: 16, note: null, alternatives: [] } }))).toBe(true)
  })
})
