...
import { emptyPlayerForm, formStateToPlayerInput, playerToFormState, validatePlayerForm } from '../playerForm'
...
describe('formStateToPlayerInput', () => {
  ...
  it('round-trips the full recovery contract through form state', () => {
    ...
    expect(input.creature_type).toEqual({ category: 'humanoid', tags: ['human'], swarm_size: null })
    ...
    expect(input.damage_resistances).toEqual([{ damage_type: 'fire', note: null, conditional: false }])
    ...
    expect(input.features?.traits).toEqual([{ name: 'Lucky', description: 'reroll a 1', attack: null }])
