...
export function PlayerEditor({ player, onClose, onSaved }: PlayerEditorProps) {
  ...
  const [form, setForm] = useState<PlayerFormState>(() =>
    player ? playerToFormState(player) : emptyPlayerForm(),
  );
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setStatus({
        message: error instanceof Error ? error.message : "Failed to save player.",
        kind: "error",
      });
      setSaving(false);
    }
        ...
        <fieldset className="player-editor-fieldset" data-region="identity">
          ...
          <div className="player-editor-grid">
            ...
            <TextField
              label="Class"
              value={form.class_}
              onChange={(e) => patch({ class_: e.target.value })}
            />
            <TextField
              label="Subclass"
              value={form.subclass}
