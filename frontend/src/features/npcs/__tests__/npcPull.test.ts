...
function monster(overrides: Partial<Monster> = {}): Monster {
  return {
    ...
    features: {
      ...
      bonus_actions: [],
      reactions: [{ name: "Tail Swipe", description: "Reaction to a nearby hit.", attack: null }],
      reaction_intro: null,
      legendary_actions: [{ name: "Wing Attack", description: "Beats wings.", attack: null }],
...
describe("applyPull", () => {
  ...
  it("selecting trait:N appends only that one trait and leaves others untouched", () => {
    const n = npc({
      features: {
        traits: [{ name: "Brave", description: "Advantage on fear saves.", attack: null }],
        spellcasting: [],
        actions: [],
        bonus_actions: [],
        reactions: [],
  ...
  it("selecting reactions:N appends the reaction and overwrites reaction_intro", () => {
    const m = monster({
      features: {
        ...monster().features,
        reactions: [{ name: "Tail Swipe", description: "Reaction.", attack: null }],
