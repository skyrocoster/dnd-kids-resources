...
describe("getAbilityScores", () => {
  ...
  it("skips a missing ability rather than emitting a placeholder", () => {
    const npc = makeNpc({
      abilities: { str: 14, dex: 12, con: null, int: null, wis: null, cha: null },
    });
    const scores = getAbilityScores(npc);
...
describe("formatSenses", () => {
  it("formats structured senses with ranges and notes", () => {
    expect(formatSenses([{ type: "darkvision", range: 60, note: "magical" }])).toEqual([
      "darkvision 60 ft. (magical)",
    ]);
  });
});
...
describe("identityLine", () => {
  it("joins race, gender, and background with a middle dot", () => {
    expect(
      identityLine(makeNpc({ race: "Human", gender: "Male", background: "Village Guard" })),
    ).toBe("Human · Male · Village Guard");
  });
...
describe("hasCombatStats", () => {
  it("is true when any of AC/HP/speed is present", () => {
    ...
    expect(
      hasCombatStats(makeNpc({ speed: [{ mode: "walk", feet: 30, note: null, hover: false }] })),
    ).toBe(true);
  });
