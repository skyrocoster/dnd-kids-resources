...
function abilityDictToText(
  values: Partial<Record<AbilityName, number>> | null | undefined,
): string {
  if (!values) return "";
  return (Object.keys(ABILITY_LABELS) as AbilityName[])
...
function textToSenses(text: string, sourceSenses: Sense[] = []): Sense[] {
  return text
    ...
    .map((line) => {
      ...
      const existing = sourceSenses.find(
        (sense) => sense.type.toLowerCase() === type.toLowerCase(),
      );
      return {
        type,
