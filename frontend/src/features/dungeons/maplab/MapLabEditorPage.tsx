...
function withObstacleLeaf(
  ...
  if (obstacle === "concealment") {
    return {
      ...
      obstacles: {
        ...state.obstacles,
        concealment: { armed: leaf.armed ?? state.obstacles.concealment.armed },
      },
    };
  }
  if (obstacle === "lock") {
    return {
      ...
      obstacles: {
        ...state.obstacles,
        lock: {
          armed: leaf.armed ?? state.obstacles.lock.armed,
          shown: leaf.shown ?? state.obstacles.lock.shown,
        },
      },
    };
  }
  return {
    ...
    obstacles: {
      ...state.obstacles,
      trap: {
        armed: leaf.armed ?? state.obstacles.trap.armed,
        shown: leaf.shown ?? state.obstacles.trap.shown,
      },
    },
  };
}
...
function authoredInspectorAdapter(
  ...
  return {
    heading: "Authored",
    onToggleOpen: (open) =>
      updateFixtureFlags(fixtureId, fixtureType, {
        state: { ...authoredFixtureState(fixture), open },
      }),
    onToggleArmed: (obstacle, armed) =>
      updateFixtureFlags(fixtureId, fixtureType, {
        state: withObstacleLeaf(fixture, obstacle, { armed }),
      }),
    onToggleShown: (obstacle, shown) =>
      updateFixtureFlags(fixtureId, fixtureType, {
        state: withObstacleLeaf(fixture, obstacle, { shown }),
      }),
  };
}
...
export function MapLabEditorPage() {
  ...
  const roomsOnActiveFloor = useMemo(
    () => roomsOnZ(state.layout, state.activeZ),
  ...
  const ghostZ = useMemo(
    () => ghostFloorZ(state.layout, state.activeZ),
    [state.layout, state.activeZ],
  );
  ...
  const activateTopFilteredTool = useCallback(() => {
    if (openFlyout === "passages" && passageToolOptions[0])
      activatePassageTool(passageToolOptions[0].key);
    if (openFlyout === "prop" && propKindOptions[0]) activatePropKind(propKindOptions[0].value);
    if (openFlyout === "terrain" && terrainToolOptions[0])
      activateTerrainTool(terrainToolOptions[0].key);
  }, [
    activatePassageTool,
    activatePropKind,
    activateTerrainTool,
  ...
  const handleRoomStrokeStart = useCallback(
    (cell: MapCell) => {
      strokeModeRef.current =
        state.selectedRoomId === null ? "create" : eraseArmed ? "erase" : "paint";
  ...
  const handleRoomStrokeCell = useCallback((cell: MapCell) => {
    setStrokeCells((cells) =>
      cells.some(([x, y]) => x === cell[0] && y === cell[1]) ? cells : [...cells, cell],
    );
  }, []);
  ...
  const roomBrushGuidance = useMemo(() => {
    ...
    return eraseArmed
      ? "Drag across squares to remove them from the room."
      : "Drag to add squares to the room.";
  ...
  const selectedDungeonRoom = useMemo(
    () =>
      selectedRoom
        ? (dungeonData.rooms?.find((room) => room.room_id === selectedRoom.room_id) ?? null)
        : null,
  ...
  const selectedFeature = useMemo(
    () =>
      state.layout.features.find((feature) => feature.feature_id === state.selectedFeatureId) ??
      null,
  ...
  const centerSelection = useCallback(
    (kind: string, id: number, force = false) => {
      const fixture =
        kind === "room"
  ...
  const centerCanvasSelection = useCallback(
    (kind: string, id: number) => {
      ...
      const isDoubleClick =
        previous?.kind === kind && previous.id === id && now - previous.at < 400;
  ...
  const selectFeatureForCanvas = useCallback(
    (id: number | null) => {
      selectFeature(id);
  ...
  const selectRoomForCanvas = useCallback(
    (id: number | null) => {
      selectRoom(id);
  ...
  const selectRoomFromFinder = useCallback(
    (roomId: number) => {
      const room = state.layout.rooms.find((candidate) => candidate.room_id === roomId);
  ...
  const selectDoorForCanvas = useCallback(
    (id: number | null) => {
      selectDoor(id);
  ...
  const selectStairForCanvas = useCallback(
    (id: number | null) => {
      selectStair(id);
  ...
  const selectPortalForCanvas = useCallback(
    (id: number | null) => {
      selectPortal(id);
  ...
  const selectPropForCanvas = useCallback(
    (id: number | null) => {
      selectProp(id);
  ...
  const selectionActions = selectedFeature ? (
    <SelectionActions
      ...
      onDelete={() =>
        deleteFixtureWithUndo(selectedFeature.title ?? selectedFeature.kind, () =>
          deleteFeature(selectedFeature.feature_id),
        )
      }
    ...
    <SelectionActions
      ...
      onDelete={() => setRoomToDelete(selectedRoom)}
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") {
        ...
        if (strokeCells.length > 0) {
          ...
          setOpenFlyout(null);
          const flyoutGroupRef =
            openFlyout === "passages"
              ? passagesFlyoutRef
              : openFlyout === "prop"
                ? propFlyoutRef
                : terrainFlyoutRef;
          flyoutGroupRef.current
            ?.querySelector<HTMLButtonElement>(".maplab-tool-palette-flyout-toggle")
            ?.focus();
  ...
  const handleAddReturnGateway = useCallback(
    (gateway: IncomingGateway) => {
      ...
      const cell: MapCell = room
        ? (absoluteCells(room).find(
            (candidate) => markersAtCell(state.layout, z, candidate).length === 0,
  ...
  if (route.status === "loading" || layoutLoading) {
    return (
      <MapLabRouteState
        title="Loading map editor"
        message="Loading dungeon layout…"
        variant="loading"
      />
    );
  }
    ...
    >
      {loadStatus.status === "empty" && (
        <p className="maplab-subtitle">
          No saved layout yet. Your first edit will save this blank map.
        </p>
      )}
      ...
      <div className="maplab-editor-layout">
        <MapLabEditorCanvas
          ...
          panHint={
            isCanvasFullscreen
              ? "Drag to pan. Pinch or scroll to zoom. Press Escape to exit fullscreen."
              : undefined
          }
          ...
          topRightSlot={(() => {
            ...
            return (
              <button
                ...
                aria-label={
                  isCanvasFullscreen ? "Exit fullscreen map editor" : "Enter fullscreen map editor"
                }
