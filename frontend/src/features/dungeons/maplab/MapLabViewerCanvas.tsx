...
export function MapLabViewerCanvas({
  ...
  onToggleRail, onCloseRoomsDrawer, onFocus, onClick, onSetActiveZ, onNavigate, onSetActiveEncounterId,
      ...
      <div className="maplab-canvas-area">
        {allLayersHidden ? <p className="maplab-canvas-filtered-empty">All layers are hidden. Turn one on to see the map.</p> : (
          ...
          >
            ...
            {layerVisible.outside && <rect className="maplab-unknown-space" x={bounds.minX * CELL_SIZE} y={bounds.minY * CELL_SIZE} width={(bounds.maxX - bounds.minX + 1) * CELL_SIZE} height={(bounds.maxY - bounds.minY + 1) * CELL_SIZE} fill="var(--maplab-outside-fill)" />}
