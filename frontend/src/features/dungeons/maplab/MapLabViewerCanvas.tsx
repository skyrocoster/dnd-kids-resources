    ...
    <div className="maplab-canvas">
      <div className="maplab-viewer-finder">
        <ViewerRoomRail
          ...
          onSelectRoom={(id) => {
            onSelectRoom(id);
            onClick({ kind: "room", id });
          }}
          open={finderOpen}
          onOpenChange={onFinderOpenChange}
        />
      </div>

      <div className="maplab-canvas-area">
        {allLayersHidden ? (
          <p className="maplab-canvas-filtered-empty">
          ...
          <MapCanvas
            ...
            bounds={bounds}
            zoom={zoom}
            ariaLabel={`Dungeon floor map — Floor ${activeZ}`}
            variant="neutral"
            ...
            bottomCenterSlot={
              viewerError ? (
                <p className="maplab-viewer-status" role="status">
                  {viewerError}
                </p>
              ) : null
            }
          ...
          >
              ...
              return (
                <g
                  ...
                  onClick={() => {
                    ...
                    onClick({ kind: "room", id: room.room_id });
                  }}
                  onKeyDown={(event) => {
                    if (event.key === "ContextMenu" || (event.key === "F10" && event.shiftKey)) {
                      event.preventDefault();
                      onFocus({ kind: "room", id: room.room_id });
                      onSelectRoom(room.room_id);
                      return;
                    }
            ...
            {layerVisible.passages &&
              doors.map((door) => (
                <DoorMarker
                  ...
                  selected={pinnedDoorId === door.door_id}
                  onFocus={() => onFocus({ kind: "door", id: door.door_id })}
                  onContextMenu={() => onFocus({ kind: "door", id: door.door_id })}
                  onClick={() => onClick({ kind: "door", id: door.door_id })}
                />
              ))}
            {layerVisible.passages && (
              ...
              stairs.map((stair) => {
                ...
                return (
                  <StairMarker
                    ...
                    selected={
                      selectedInspectable?.kind === "stair" &&
                      selectedInspectable.id === stair.stair_id
                    }
                    onFocus={() => onFocus({ kind: "stair", id: stair.stair_id })}
                    onContextMenu={() => onFocus({ kind: "stair", id: stair.stair_id })}
                    onClick={() => {
                      onClick({ kind: "stair", id: stair.stair_id });
                      onNavigateStair(stair);
                    }}
                  />
                );
              })}
              ...
              portals.map((portal) => {
                ...
                return (
                  <PortalMarker
                    ...
                    simplified={simplified}
                    selected={
                      selectedInspectable?.kind === "portal" &&
                      selectedInspectable.id === portal.portal_id
                    }
                    onFocus={() => onFocus({ kind: "portal", id: portal.portal_id })}
                    onContextMenu={() => onFocus({ kind: "portal", id: portal.portal_id })}
                    onClick={() => {
                      onClick({ kind: "portal", id: portal.portal_id });
            ...
            {layerVisible.props &&
              props.map((prop) => {
                const propOffset =
                  prop.side === undefined
                    ? markerOffset(layout, activeZ, prop.cell, "prop", prop.prop_id)
                    : undefined;
                return (
                  <PropMarker
                    key={prop.prop_id}
                    ...
                    simplified={simplified}
                    selected={
                      selectedInspectable?.kind === "prop" &&
                      selectedInspectable.id === prop.prop_id
                    }
                    onFocus={() => onFocus({ kind: "prop", id: prop.prop_id })}
                    onContextMenu={() => onFocus({ kind: "prop", id: prop.prop_id })}
                    onClick={() => {
                      onClick({ kind: "prop", id: prop.prop_id });
                      if (prop.kind === "encounter" && prop.encounter_id != null)
                        onSetActiveEncounterId(prop.encounter_id);
                    }}
                  />
                );
              })}
