...
export function togglePortalLocked(portalSessions: SessionMap, portal: MapPortal): SessionFixtureState {
...
export function disarmPortalTrap(portalSessions: SessionMap, portal: MapPortal): SessionFixtureState {
  const currentOverride = portalSessions[portal.portal_id]
  return {
    ...(currentOverride ?? {}),
    obstacles: {
...
export function buildPreviewSessionMap(
  ...
  return {
    doors: Object.fromEntries(layout.doors.map((d) => [String(d.door_id), doorSessions[d.door_id] ?? {} as SessionFixtureState])),
    stairs: Object.fromEntries(layout.stairs.map((s) => [String(s.stair_id), stairSessions[s.stair_id] ?? {} as SessionFixtureState])),
    portals: Object.fromEntries(layout.portals.map((p) => [String(p.portal_id), portalSessions[p.portal_id] ?? {} as SessionFixtureState])),
