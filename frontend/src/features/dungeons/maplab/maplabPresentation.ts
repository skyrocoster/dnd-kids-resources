...
function passageStateChipsForDescriptor(passage: PassageFlags): PassageStateChip[] {
  return PASSAGE_STATE_PRECEDENCE.filter(
    ...
    state,
    icon: PASSAGE_STATE_CHIP_ICONS[state],
    label: PASSAGE_STATE_CHIP_LABELS[state],
  }));
}
...
function passageDescriptorLines(passage: PassageFlags): { label: string; value: string }[] {
  ...
  if (passage.breakDc !== undefined)
    lines.push({ label: "Break DC", value: String(passage.breakDc) });
  if (passage.pickDc !== undefined) lines.push({ label: "Pick DC", value: String(passage.pickDc) });
  if (passage.hiddenDc !== undefined)
    lines.push({ label: "Perception DC", value: String(passage.hiddenDc) });
  if (passage.searchDc !== undefined)
    lines.push({ label: "Search DC", value: String(passage.searchDc) });
      ...
      lines.push(
        portal.to?.dungeon_id !== undefined
          ...
          : portal.to?.cell && portal.to.z !== undefined
            ? {
                label: "Leads to",
                value: `${portal.to.cell[0]},${portal.to.cell[1]} (z:${portal.to.z})`,
              }
            : {
                label: "Destination",
...
function doorPresentationForDescriptor(
  door: MapDoor,
...
export function fixtureStateChips(effective: FixtureState): FixtureStateChip[] {
  ...
  if (effective.obstacles.concealment.armed)
    chips.push({
      state: "concealed",
      icon: FIXTURE_STATE_CHIP_ICONS.concealed,
      label: FIXTURE_STATE_CHIP_LABELS.concealed,
    });
  if (effective.obstacles.trap.armed)
    chips.push({
      state: "trapped",
      icon: FIXTURE_STATE_CHIP_ICONS.trapped,
      label: FIXTURE_STATE_CHIP_LABELS.trapped,
    });
  if (effective.obstacles.lock.armed)
    chips.push({
      state: "locked",
      icon: FIXTURE_STATE_CHIP_ICONS.locked,
      label: FIXTURE_STATE_CHIP_LABELS.locked,
    });
  return chips;
}
...
function fixtureDescriptorLines(
  ...
  if (passage.breakDc !== undefined && armedLockOrTrap)
    lines.push({ label: "Break DC", value: String(passage.breakDc) });
  if (passage.pickDc !== undefined && effective.obstacles.lock.armed)
    lines.push({ label: "Pick DC", value: String(passage.pickDc) });
  if (passage.hiddenDc !== undefined && effective.obstacles.concealment.armed)
    lines.push({ label: "Perception DC", value: String(passage.hiddenDc) });
  if (passage.searchDc !== undefined && effective.obstacles.concealment.armed)
    lines.push({ label: "Search DC", value: String(passage.searchDc) });
  if (passage.note) lines.push({ label: "Note", value: passage.note });
  return lines;
}
