...
describe("MapLabPage (Stage 3 — Generic inspector)", () => {
  it("shows the room descriptor (title, size, description) in the same panel when selected", async () => {
    ...
    expect(panel.querySelector(".maplab-inspector-title")).toHaveTextContent(
...
describe("MapLabPage (Stage F2 — Prop rendering)", () => {
  ...
  it("renders an on-wall prop anchored at the wall midpoint, smaller than an on-square prop", async () => {
    ...
    expect(Number(mirrorCircle.getAttribute("r"))).toBeLessThan(
      Number(chestCircle.getAttribute("r")),
    );
  });
...
describe("MapLabPage (R4 viewer room-reading surface)", () => {
  it("shows room details after selecting the default active room", async () => {
    ...
    expect(
      within(details).getByRole("heading", { name: "Training Hall", level: 3 }),
    ).toBeInTheDocument();
  ...
  it("stair travel updates the room details on the destination floor", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "First Floor Landing", level: 3 }),
    ).toBeInTheDocument();
    expect(screen.getByText("Balcony")).toBeInTheDocument();
  });
  ...
  it("fixture inspection still works while the details panel shows the active room", async () => {
    ...
    expect(screen.getByText("Door")).toBeInTheDocument();
    expect(
      within(screen.getByLabelText("Room details")).getByRole("heading", {
        name: "Training Hall",
        level: 3,
      }),
    ).toBeInTheDocument();
  ...
  it("data-only rooms are not invented on the map", async () => {
    await renderLoadedMapLabPage();
    expect(
      within(screen.getByRole("group", { name: /dungeon floor map/i })).queryByRole("button", {
        name: "Data Only Room",
      }),
