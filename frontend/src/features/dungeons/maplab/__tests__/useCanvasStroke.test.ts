...
describe('useCanvasStroke', () => {
  it('start reports one cell', () => {
    ...
    act(() => result.current.onStrokePointerDown(makePointerEvent({ pointerId: 1, clientX: 0, clientY: 0 })))
  ...
  it('a drag across three cells reports each exactly once, in order', () => {
    ...
    act(() =>
      result.current.onStrokePointerMove(makePointerEvent({ pointerId: 1, clientX: 64, clientY: 0 })),
    )
    ...
    act(() =>
      result.current.onStrokePointerMove(makePointerEvent({ pointerId: 1, clientX: 128, clientY: 0 })),
    )
    ...
    act(() =>
      result.current.onStrokePointerMove(makePointerEvent({ pointerId: 1, clientX: 192, clientY: 0 })),
    )
  ...
  it('a second pointer aborts the stroke', () => {
    ...
    act(() => result.current.onStrokePointerDown(makePointerEvent({ pointerId: 2, clientX: 0, clientY: 64 })))
  ...
  it('ignores non-primary pointer', () => {
    ...
    act(() => result.current.onStrokePointerDown(makePointerEvent({ pointerId: 1, button: 2, clientX: 0, clientY: 0 })))
    expect(onStrokeStart).not.toHaveBeenCalled()
  })
