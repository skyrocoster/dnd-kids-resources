...
describe("MapLabPage (Stage 4 — Passage session state)", () => {
  ...
  it("disarms traps and reflects the change in the passage glyph", async () => {
    ...
    expect(door).toHaveAttribute("data-state", "trapped");
    expect(
      document.querySelector('.maplab-door-badge-layer [data-badge="trap-disarmed"]'),
    ).not.toBeInTheDocument();
    expect(
      document.querySelector('.maplab-door-badge-layer [data-badge="multiple-statuses"]'),
    ).toBeInTheDocument();
...
describe("MapLabPage (Stage 03 — viewer status chip action failures)", () => {
  ...
  it("clears a stale save error when the next passage toggle succeeds", async () => {
    ...
    expect(
      await screen.findByText("Couldn't save session changes. Try again."),
    ).toBeInTheDocument();
  ...
  it("clears a stale at-table error when a door is toggled", async () => {
    ...
    expect(
      screen.queryByText("Couldn't put this map at the table. Try again."),
    ).not.toBeInTheDocument();
  });
  ...
  it("Party is here persists the selected room as partyRoomId", async () => {
    const user = userEvent.setup();
    const saveSpy = vi
      .spyOn(api, "saveDungeonSessionState")
      .mockResolvedValue(undefined as unknown as { data: Record<string, unknown> });
    renderMapLabPage();
  ...
  it("marks the persisted party room in the DM map view", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /Training Hall.*party location/i }),
      ).toBeInTheDocument();
    });

    expect(screen.getByRole("button", { name: /Training Hall.*party location/i })).toHaveAttribute(
      "data-party",
      "true",
    );
  });
