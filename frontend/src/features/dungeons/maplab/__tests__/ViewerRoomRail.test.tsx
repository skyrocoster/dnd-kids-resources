...
const parsed = parseDungeonData({
  rooms: [
    ...
    },
    {
      ...
      title: "East Wing",
      entries: [
        { entry_type: "monster", title: "Watcher", content: "A monster hides in the rafters." },
      ],
      npcs: [],
    },
  ],
...
describe("ViewerRoomRail", () => {
  ...
  it("groups rooms by floor with headings for multi-floor layouts", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Two-Wing Test Layout", level: 4 }),
    ).toBeInTheDocument();
  ...
  it("keeps rooms in roomsOnZ order within a floor", () => {
    ...
    const groundButtons = within(
      screen.getByRole("listbox", { name: "Ground Floor rooms" }),
    ).getAllByRole("button");
    ...
    const layout = {
      ...
      rooms: [
        ...mapLabLayout.rooms,
        {
          room_id: 101,
          z: 0,
          origin: [20, 0] as [number, number],
          cells: [[0, 0] as [number, number]],
          title: "Layout Spare Room",
        },
      ],
    };
  ...
  it("skips floors with no rooms", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Empty Floor", level: 4 }),
  ...
  it("omits the floor heading when only one populated floor exists", () => {
    ...
    render(
      <ViewerRoomRail
        layout={layout}
        parsed={parseDungeonData({ rooms: [{ room_id: 17, title: "Lone Room" }] })}
        activeRoomId={17}
        onSelectRoom={vi.fn()}
      />,
    );
          ...
          entries: [
            {
              ...
              content: "Darts fire from the walls.",
            },
            {
              entry_type: "encounter",
              title: "Goblin Drill",
          ...
          entries: [
            {
              entry_type: "feature",
              title: "Weapon Racks",
              content: "Dusty weapons line the walls.",
            },
          ],
          npcs: [],
        },
  ...
  it("filters by room number, title, and floor and labels off-map results", async () => {
    ...
    const layout = {
      ...mapLabLayout,
      rooms: mapLabLayout.rooms.filter((room) => room.room_id !== 100),
    };
    render(
      <ViewerRoomRail layout={layout} parsed={parsed} activeRoomId={17} onSelectRoom={vi.fn()} />,
    );
