 ...
 * flyout, then clicks the named tool button, mirroring how a person actually reaches this tool. */
...
describe("MapLabEditorPage", () => {
  ...
  it("loads a blank layout when the backend has no saved layout (404) and does not save until edited", async () => {
    ...
    expect(
      screen.getByText("No saved layout yet. Your first edit will save this blank map."),
    ).toBeInTheDocument();
  ...
  it("shows room content editing and persists room title changes", async () => {
    ...
    const updateDungeonSpy = vi
      .spyOn(api, "updateDungeon")
      .mockResolvedValue({ id: 4, title: "Test Dungeon", data: {} });
    vi.spyOn(api, "saveDungeonLayout").mockResolvedValue({ data: layout });
  ...
  it("shows the create-room-data action for layout-only rooms", async () => {
    ...
    expect(updateDungeonSpy).toHaveBeenCalledTimes(1);
    const payload = updateDungeonSpy.mock.calls[0][1].data as {
      rooms: Array<{ room_id: number; title: string }>;
    };
