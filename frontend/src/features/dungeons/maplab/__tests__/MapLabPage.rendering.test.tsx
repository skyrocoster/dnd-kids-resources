...
describe("MapLabPage (M2 stairs + second floor)", () => {
  it("renders both floor choices, starting on the ground floor", async () => {
    await renderLoadedMapLabPage();
    const groundFloor = screen.getByRole("button", { name: "Ground Floor" });
    const firstFloor = screen.getByRole("button", { name: "First Floor" });
  ...
  it("switches the single active-floor canvas via the floor choices", async () => {
    const user = userEvent.setup();
    await renderLoadedMapLabPage();
    ...
    const activeCanvas = screen.getByRole("group", { name: /dungeon floor map/i });
    expect(
      within(activeCanvas).getByRole("button", { name: "First Floor Landing" }),
    ).toBeInTheDocument();
    expect(screen.getAllByRole("group", { name: /dungeon floor map/i })).toHaveLength(1);
...
describe("MapLabPage (M2.2 grid canvas + scale)", () => {
  ...
  it("renders the Armoury's L-shaped footprint without the notch cell", async () => {
    renderMapLabPage();
    await flush();
    const armoury = within(screen.getByRole("group", { name: /dungeon floor map/i })).getByRole(
      "button",
      { name: "Armoury" },
    );
...
describe("MapLabPage (Stage 1 — Faithful L-shape rendering)", () => {
  ...
  it("renders the interlocking-L test pair on z:2 with no overlap", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Two-Wing Test Layout" }));
...
describe("MapLabPage (Stage 2 — Passage visuals)", () => {
  ...
  describe("MapLabPage (Stage E1 — Unified data: viewer reads backend layout)", () => {
    ...
    it("404 from backend falls back to the fixture layout", async () => {
      ...
      expect(
        screen.queryByRole("button", { name: "Combat Training Hall" }),
