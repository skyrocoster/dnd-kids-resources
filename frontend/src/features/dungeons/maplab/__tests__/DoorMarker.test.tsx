...
function renderDoor(marker: MapDoor, isOpen = true) {
  return render(
    <svg>
      ...
      <g className="maplab-door-badge-layer">
        <DoorBadgeLayer door={marker} cellSize={40} session={{ open: isOpen }} />
      </g>
    </svg>,
  );
}
...
describe("DoorBadgeLayer collapsed status (M3)", () => {
  it("renders exactly one badge disc for multiple active flags", () => {
    ...
    expect(container.querySelector(".maplab-door-badge")).toHaveAttribute(
      "data-badge",
      "multiple-statuses",
    );
  });
...
describe("DoorMarker", () => {
  ...
  it("uses the badge role foreground and places the status disc on the leaf", () => {
    ...
    expect(container.querySelector(".maplab-door-badge svg")).toHaveStyle({
    ...
    const { container } = render(
      <svg>
        <DoorMarker
          door={marker}
          cellSize={40}
          session={{ obstacles: { trap: { armed: false } }, open: true }}
        />
        <g className="maplab-door-badge-layer">
          <DoorBadgeLayer
            door={marker}
