...
describe("markerBadges", () => {
  it("returns chips from passageStateChips in precedence order", () => {
    expect(
      markerBadges(prop({ hidden: true, locked: true, trapped: true })).map((badge) => badge.key),
    ).toEqual(["trapped", "locked", "hidden"]);
  });

  it("adds loot and trapDisarmed after passage-state badges", () => {
    expect(
      markerBadges(prop({ locked: true, trapped: true, loot: { bundle_id: 1 } }), true).map(
        (badge) => badge.key,
      ),
    ).toEqual(["trapped", "locked", "loot", "trap-disarmed"]);
  });
...
describe("fixtureMarkerBadges (DM policy)", () => {
  it("emits armed obstacle vocabulary regardless of Shown", () => {
    ...
    expect(fixtureMarkerBadges(fixture).map((badge) => badge.key)).toEqual([
      "concealed",
      "trapped",
      "locked",
    ]);
...
describe("collapsedStatusDescriptor (M1)", () => {
  ...
  it.each([
    ...
    [
      "locked + hidden + loot",
      prop({ hidden: true, locked: true, loot: { bundle_id: 1 } }),
      false,
      ["locked", "hidden", "loot"],
    ],
    ...
    [
      "all states",
      prop({ hidden: true, locked: true, trapped: true, loot: { bundle_id: 1 } }),
      true,
      ["trapped", "locked", "hidden", "loot", "trap-disarmed"],
    ],
  ] as const)(
    "returns one synthetic descriptor for %s",
    (_name, source, trapDisarmed, expectedKeys) => {
...
describe("boundedBadgeLayout (M2)", () => {
  ...
  function placementFor(count: number, index: number) {
    ...
    const markerRadius =
      cellSize * (count > 1 ? GROUPED_MARKER_RADIUS_FRACTION : MARKER_RADIUS_FRACTION);
    ...
    for (let a = 0; a < placements.length; a += 1) {
      for (let b = a + 1; b < placements.length; b += 1) {
        const distance = Math.hypot(
          placements[a].cx - placements[b].cx,
          placements[a].cy - placements[b].cy,
        );
  ...
  it("clamps edge-biased marker centers to keep the full disc inside the cell", () => {
    expect(boundedBadgeLayout(40, 40, cellSize, 39, 81, 12, badgeRadius)).toEqual({
      cx: 48,
      cy: 72,
      radius: 8,
    });
