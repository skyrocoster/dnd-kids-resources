...
describe("PropMarker bounded badge (M2)", () => {
  it("renders one status disc with its own icon for a single active flag", () => {
    ...
    expect(container.querySelector('[data-badge="locked"] svg')).toHaveStyle({
      color: "var(--md-on-passage-locked)",
    });
  });
  ...
  it("renders Layers icon disc for multiple active flags", () => {
    ...
    expect(container.querySelector('[data-badge="multiple-statuses"] svg')).toHaveStyle({
  ...
  it("names the Layers disc and narrates every active status in the ARIA label", () => {
    ...
    expect(getByRole("button").getAttribute("aria-label")).toMatch(
  ...
  it("uses encounter identity for encounter props regardless of status", () => {
    ...
    expect(container.querySelector(".maplab-prop-marker")).toHaveStyle({
      stroke: "var(--md-tertiary)",
    });
    expect(container.querySelector(".maplab-prop-icon")).toHaveStyle({
  ...
  it("renders one collapsed disc and narrates every active badge", () => {
    const { container, getByRole } = renderMarker(
      prop({ locked: true, trapped: true, loot: { bundle_id: 1 } }),
    );
    ...
    expect(
      getByRole("button", {
        name: "Treasure chest — Multiple statuses: Trapped, Locked, Loot assigned",
      }),
    ).toBeTruthy();
  });
