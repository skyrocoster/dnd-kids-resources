...
const dungeonDataFixture = {
  rooms: [
      ...
      entries: [
        {
          entry_type: "feature",
          title: "Banner",
          content: "Ancient banners hang above the arena.",
        },
        { entry_type: "trap", title: "Loose Flagstones", content: "Creatures trigger 1d6 darts." },
        {
          entry_type: "encounter",
          title: "Goblin Drill",
          content: "2d4 goblins rush the room.",
          encounter_id: 7,
        },
        {
          entry_type: "treasure",
          title: "Hidden Cache",
          content: "A niche in the wall.",
    ...
    },
    {
      ...
      title: "Armoury",
      entries: [
        { entry_type: "feature", title: "Weapon Racks", content: "Dusty weapons line the walls." },
      ],
      npcs: [],
    },
    {
      ...
      entries: [
        {
          entry_type: "feature",
          title: "Balcony",
          content: "A narrow overlook faces the courtyard.",
        },
      ],
      npcs: [],
    },
    {
      ...
      entries: [
        { entry_type: "feature", title: "Ghost Note", content: "No geometry should render this." },
      ],
      npcs: [],
    },
  ],
...
beforeEach(() => {
  ...
  vi.spyOn(api, "getDungeonSessionState").mockRejectedValue(
    new api.ApiError(404, "Session state not found"),
  );
  vi.spyOn(api, "saveDungeonSessionState").mockResolvedValue(
...
describe("MapLabPage (R5 viewer navigation rail)", () => {
  ...
  it("shows populated floor groups in the rail", async () => {
    ...
    expect(
      within(screen.getByRole("dialog", { name: "Find room" })).getByRole("button", {
        name: /Training Hall/i,
      }),
    ).toBeInTheDocument();
  ...
  it("floor choice switches keep the rail selection synchronized", async () => {
    const user = userEvent.setup();
    await renderLoadedMapLabPage();
...
describe("MapLabPage portal viewer rendering and navigation", () => {
  ...
  it("renders a portal on its authored floor and not on the other floor", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /Shimmering Archway \(return\)/i }),
  ...
  it("clears a selected viewer inspectable when empty space is clicked", async () => {
    ...
    const mapArmoury = within(screen.getByRole("group", { name: /dungeon floor map/i })).getByRole(
      "button",
      { name: "Armoury" },
    );
  ...
  it("clicking a portal jumps the active floor to its destination z", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Shimmering Archway \(return\) —/i }),
    ).toBeInTheDocument();
  });
  ...
  it("frames a resolved stair destination without travelling twice", async () => {
    ...
    vi.spyOn(api, "getDungeonLayout").mockResolvedValue({
      data: { ...mapLabLayout, stairs: [stair] },
    });
  ...
  it("Escape dismisses one viewer owner per press in priority order before clearing selection", async () => {
    const user = userEvent.setup();
    const encounterProp = {
      prop_id: 503,
      kind: "encounter",
    ...
    vi.spyOn(api, "getDungeonLayout").mockResolvedValue({
      data: { ...mapLabLayout, props: [...mapLabLayout.props, encounterProp] },
    });
    ...
    const { container } = await renderLoadedMapLabPage();
    const map = screen.getByRole("group", { name: /dungeon floor map/i });
    await user.click(within(map).getByRole("button", { name: "Armoury" }));
    const inspector = container.querySelector(".maplab-inspector-panel-container")!;
    expect(inspector.querySelector(".maplab-inspector-title")).toHaveTextContent("Armoury");
    ...
    await user.click(screen.getByRole("button", { name: /Goblin Ambush/i }));
    await screen.findByRole("dialog", { name: "Goblin Ambush" });
    await user.keyboard("{Escape}");
    expect(screen.queryByRole("dialog", { name: "Goblin Ambush" })).not.toBeInTheDocument();
    expect(inspector.querySelector(".maplab-inspector-title")).toHaveTextContent("Goblin Ambush");

    await user.click(within(map).getByRole("button", { name: "Combat Training Hall" }));
    expect(inspector.querySelector(".maplab-inspector-title")).toHaveTextContent("Training Hall");
    await user.click(screen.getByRole("button", { name: "Mira" }));
    await screen.findByRole("dialog", { name: "Mira" });
    await user.keyboard("{Escape}");
    expect(screen.queryByRole("dialog", { name: "Mira" })).not.toBeInTheDocument();
    expect(inspector.querySelector(".maplab-inspector-title")).toHaveTextContent("Training Hall");

    await user.click(screen.getByRole("button", { name: "Reset dungeon" }));
    expect(
      screen.getByRole("alertdialog", {
        name: 'Reset "Test Dungeon"? Every door, trap, and toggle returns to its authored state. This cannot be undone.',
      }),
    ).toBeInTheDocument();
    await user.keyboard("{Escape}");
    expect(
  ...
  it("a co-located stair and portal render as distinct, non-overlapping markers", async () => {
    ...
    const backendLayout = {
      ...mapLabLayout,
      stairs: [...mapLabLayout.stairs, stair],
      portals: [colocatedPortal, pairedPortal],
    };
    vi.spyOn(api, "getDungeonLayout").mockResolvedValue({ data: backendLayout });
...
describe("VT0 — Viewer room finder", () => {
  ...
  it("floor choices remain visible when the drawer is open", async () => {
    await renderLoadedMapLabPage();
  ...
  it("floor choices live inside the toolbar and outside the finder in floor order", async () => {
    await renderLoadedMapLabPage();
    const toolbar = document.querySelector(".maplab-toolbar");
    const choices = screen.getByRole("group", { name: "Dungeon floors" });
    expect(toolbar).toContainElement(choices);
    expect(document.querySelector(".maplab-viewer-finder")).not.toContainElement(choices);

    const viewButton = screen.getByRole("button", { name: "View" });
    const groundFloor = within(choices).getByRole("button", { name: "Ground Floor" });
    const firstFloor = within(choices).getByRole("button", { name: "First Floor" });
    expect(
      viewButton.compareDocumentPosition(groundFloor) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      groundFloor.compareDocumentPosition(firstFloor) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(groundFloor).toHaveAccessibleName("Ground Floor");
    expect(firstFloor).toHaveAccessibleName("First Floor");
  });
  ...
  it("room buttons inside the finder meet the 48px touch floor", async () => {
    ...
    await renderLoadedMapLabPage();
    await user.click(screen.getByRole("button", { name: "Find room…" }));
    const finder = screen.getByRole("dialog", { name: "Find room" });
    const roomButton = within(finder).getByRole("button", { name: "Armoury" });
