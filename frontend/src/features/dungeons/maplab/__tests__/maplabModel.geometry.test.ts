...
describe("padding migration", () => {
  it("converts old single-number padding to per-side padding", () => {
    const oldLayout = {
      meta: {
        cellSizeFt: 5,
        padding: 3 as unknown as { top: number; right: number; bottom: number; left: number },
      },
      rooms: [],
      doors: [],
      stairs: [],
...
describe("roomsOnZ", () => {
  it("returns only rooms on the ground floor (z:0)", () => {
    const ids = roomsOnZ(mapLabLayout, 0)
      .map((r) => r.room_id)
      .sort();
...
describe("doorsOnFloor / propsOnFloor — floor-stacked coordinates", () => {
  ...
  it("falls back to spatial inference when z is absent (legacy data)", () => {
    ...
    expect(
      doorsOnFloor({ ...legacyLayout, rooms: [legacyLayout.rooms[0]] }, 0).map((d) => d.door_id),
    ).toEqual([3]);
  });
