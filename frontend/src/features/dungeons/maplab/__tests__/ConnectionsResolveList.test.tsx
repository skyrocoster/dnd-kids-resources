...
describe("ConnectionsResolveList", () => {
  it("renders one row per unresolved portal", () => {
    ...
    expect(chooseButton).toHaveClass(
      "maplab-pill-button",
      "maplab-connections-resolve-list-action",
    );
  ...
  it("shows the empty-state copy when nothing is unresolved", () => {
    ...
    expect(
      screen.getByText("Every connection has both ends. Nothing to resolve."),
    ).toBeInTheDocument();
  });
  ...
  it("renders a repoint/remove row for a gateway whose target dungeon no longer exists", async () => {
    ...
    expect(repointButton).toHaveClass(
      "maplab-pill-button",
      "maplab-connections-resolve-list-action",
    );
  ...
  it("renders an add-return-gateway row for an incoming gateway with no return", async () => {
    ...
    expect(addReturnButton).toHaveClass(
      "maplab-pill-button",
      "maplab-connections-resolve-list-action",
    );
  ...
  it("shows only the error panel when connections fail to load", () => {
    ...
    expect(
      screen.queryByText("Every connection has both ends. Nothing to resolve."),
  ...
  it("does not flag an incoming gateway that already has a return portal", () => {
    renderList({
      layout: layoutWithPortals([
        { ...validGatewayPortal, to: { dungeon_id: incomingGateway.dungeon_id } },
      ]),
      dungeons: [
        { id: incomingGateway.dungeon_id, title: incomingGateway.dungeon_title, data: {} },
      ],
      incomingGateways: [incomingGateway],
    });
