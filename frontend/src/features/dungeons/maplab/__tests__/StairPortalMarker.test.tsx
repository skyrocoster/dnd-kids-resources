...
describe.skip('StairMarker bounded badge (M2)', () => {
  ...
  it('keeps identity stroke stable regardless of status', () => {
    ...
    expect(unlocked.querySelector('.maplab-stair-marker')).toBeTruthy()
    expect(locked.querySelector('.maplab-stair-marker')).toBeTruthy()
  })

  it('keeps hidden dashed outline', () => {
    const { container } = render(<svg><StairMarker stair={stair({ hidden: true })} cellSize={40} cell={[1, 1]} activeZ={1} /></svg>)
...
describe.skip('PortalMarker bounded badge (M2)', () => {
  it('renders one disc for a single flag, Layers for multiple flags', () => {
  ...
  it('keeps identity stroke stable regardless of status', () => {
    ...
    expect(unlocked.querySelector('.maplab-portal-marker')).toBeTruthy()
    expect(locked.querySelector('.maplab-portal-marker')).toBeTruthy()
  })

  it('keeps hidden dashed outline', () => {
    const { container } = render(<svg><PortalMarker portal={portal({ hidden: true })} cellSize={40} /></svg>)
...
describe('StairMarker', () => {
  it('renders every active and disarmed badge in a non-overlapping ring', () => {
    ...
    expect(new Set([...badges].map((badge) => badge.getAttribute('transform'))).size).toBe(3)
    expect(getByRole('button', { name: 'Stair 1 — Trapped, Locked, Trap disarmed' })).toBeTruthy()
  })

  it('keeps the badge ring centered on a grouped marker offset', () => {
    ...
    expect(container.querySelector('.maplab-badge')).toHaveAttribute('transform', 'translate(68, 38.8)')
  })
})

describe('PortalMarker', () => {
  it('renders every active and disarmed badge in a radial ring', () => {
    const { container, getByRole } = render(
      <svg>
