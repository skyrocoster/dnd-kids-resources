...
describe("StairMarker bounded badge (M2)", () => {
  it("renders one disc for a single flag, Layers for multiple flags", () => {
    const { container: single } = render(
      <svg>
        <StairMarker
          stair={stair({ state: lockedFixture })}
          cellSize={40}
          cell={[1, 1]}
          activeZ={1}
        />
  ...
  it("renders no disc when no flags are active", () => {
    const { container } = render(
      <svg>
        <StairMarker stair={stair()} cellSize={40} cell={[1, 1]} activeZ={1} />
      </svg>,
    );
  ...
  it("keeps identity stroke stable regardless of status", () => {
    ...
    expect(unlocked.querySelector(".maplab-stair-marker")).toHaveStyle({
      stroke: "var(--md-tertiary)",
    });
    expect(locked.querySelector(".maplab-stair-marker")).toHaveStyle({
      stroke: "var(--md-tertiary)",
    });
    expect(unlocked.querySelector(".maplab-stair-icon")).toHaveStyle({
  ...
  it("keeps concealment dashed outline", () => {
    const { container } = render(
      <svg>
        <StairMarker
          stair={stair({ state: concealedLockedFixture })}
          cellSize={40}
          cell={[1, 1]}
          activeZ={1}
        />
...
describe("PortalMarker bounded badge (M2)", () => {
  it("renders one disc for a single flag, Layers for multiple flags", () => {
    const { container: single } = render(
      <svg>
        <PortalMarker portal={portal({ state: lockedFixture })} cellSize={40} />
      </svg>,
    );
    const { container: multi } = render(
      <svg>
        <PortalMarker portal={portal({ state: lockedTrappedFixture })} cellSize={40} />
      </svg>,
    );
  ...
  it("renders no disc when no flags are active", () => {
    const { container } = render(
      <svg>
        <PortalMarker portal={portal()} cellSize={40} />
      </svg>,
    );
    expect(container.querySelector(".maplab-badge")).toBeNull();
  });
  ...
  it("keeps identity stroke stable regardless of status", () => {
    ...
    expect(unlocked.querySelector(".maplab-portal-marker")).toHaveStyle({
      stroke: "var(--md-primary)",
    });
    expect(locked.querySelector(".maplab-portal-marker")).toHaveStyle({
      stroke: "var(--md-primary)",
    });
    expect(unlocked.querySelector(".maplab-portal-icon")).toHaveStyle({
  ...
  it("keeps concealment dashed outline", () => {
    const { container } = render(
      <svg>
        <PortalMarker portal={portal({ state: concealedLockedFixture })} cellSize={40} />
      </svg>,
    );
    ...
    const { container } = render(
      <svg>
        <StairMarker
          ...
          cell={[1, 1]}
          activeZ={1}
          offset={{ dx: 0.2, dy: -0.1 }}
          grouped
        />
      </svg>,
    );

    expect(container.querySelector(".maplab-badge")).toHaveAttribute(
      "transform",
      "translate(68, 56)",
    );
  });
...
describe("PortalMarker", () => {
  ...
  it("keeps the collapsed badge in the grouped marker sub-slot", () => {
    const { container } = render(
      <svg>
        <PortalMarker
          portal={portal({ state: lockedFixture })}
          cellSize={40}
          offset={{ dx: 0.2, dy: -0.1 }}
          grouped
        />
