...
function validTarget(value: unknown): value is MapLabNavigationTarget {
  ...
  return typeof target.kind === 'string' && Number.isInteger(target.id) && (target.id as number) >= 0
}
...
function validZoom(value: unknown): value is ZoomState {
  ...
  const pan = zoom.pan
  return typeof zoom.scale === 'number' && Number.isFinite(zoom.scale) && zoom.scale >= 0.25 && zoom.scale <= 3
    && !!pan && typeof pan === 'object'
    && typeof (pan as Record<string, unknown>).x === 'number' && Number.isFinite((pan as Record<string, unknown>).x)
    && typeof (pan as Record<string, unknown>).y === 'number' && Number.isFinite((pan as Record<string, unknown>).y)
}
...
export function decodeMapLabNavigation(value: string | null): MapLabNavigationState | null {
  ...
  try {
    ...
    return {
      ...
      zoom: parsed.zoom,
      selectedTarget: validTarget(parsed.selectedTarget)
        ? parsed.selectedTarget
        : null,
      focusTarget: validTarget(parsed.focusTarget)
        ? parsed.focusTarget
        : null,
    }
  } catch {
...
export function useMapLabNavigationSession(dungeonId: number | null) {
  ...
  useEffect(() => {
    ...
    try { restored = decodeMapLabNavigation(window.sessionStorage.getItem(key)) } catch { /* session storage is optional */ }
  ...
  useEffect(() => {
    ...
    try { window.sessionStorage.setItem(key, JSON.stringify(state)) } catch { /* session storage is optional */ }
