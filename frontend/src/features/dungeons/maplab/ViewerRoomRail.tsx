...
export function ViewerRoomRail({
  layout,
  parsed,
  activeRoomId,
  onSelectRoom,
  ...
  const activeFloor = floorGroups.find((group) =>
    group.rooms.some((room) => room.room_id === activeRoomId),
  )?.floor.z;
  const offMapRooms = useMemo(
    () =>
      (parsed.rooms ?? []).filter(
        (room) => !layout.rooms.some((layoutRoom) => layoutRoom.room_id === room.room_id),
      ),
  ...
  const matchesRoom = (roomId: number, title: string, floorTitle: string) =>
    !normalizedQuery ||
    `${roomId} ${title} ${floorTitle}`.toLocaleLowerCase().includes(normalizedQuery);
  const orderedFloorGroups = useMemo(
    () =>
      [...floorGroups].sort(
        (a, b) => Number(b.floor.z === activeFloor) - Number(a.floor.z === activeFloor),
      ),
    ...
    >
      <div className="maplab-viewer-rail" role="navigation" aria-label="Room navigation">
        <Popover.Trigger
          ref={triggerRef}
          type="button"
          className="maplab-viewer-rail-trigger"
        ...
        <Popover.Positioner side="bottom" align="start" className="maplab-viewer-rail-positioner">
          ...
          >
            <div className="maplab-viewer-rail-search-row">
              ...
              <Button
                type="button"
                className="maplab-viewer-rail-close"
                onClick={close}
            ...
            <input
              ref={searchRef}
              id="maplab-room-search"
              type="search"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search by number, title, or floor"
            />
            <p className="maplab-viewer-rail-result-summary" aria-live="polite">
              Results from all floors
            </p>
            {orderedFloorGroups.map(({ floor, rooms }) => {
              ...
              const visibleRooms = rooms.filter((room) => {
                ...
                return matchesRoom(
                  room.room_id,
                  dataRoom?.title ?? room.title ?? `Room ${room.room_id}`,
                  floorTitle,
                );
              });
              ...
              return (
                <section key={floor.z} className="maplab-viewer-rail-floor">
                  {showFloorTitles && (
                    <h4 className="maplab-viewer-rail-floor-title">{floorTitle}</h4>
                  )}
                  <ul
                    className="maplab-viewer-rail-room-list"
                    role="listbox"
                    aria-label={`${floorTitle} rooms`}
                  >
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="maplab-viewer-rail-room-number" aria-hidden="true">
                              #{room.room_id}
                            </span>
                            {(hasHints || hasNpcHint) && (
                              <span
                                className="maplab-viewer-rail-room-hints"
                                aria-label="Room hints"
                              >
                                {threatHints?.hasTrap && (
                                  <span className="maplab-viewer-rail-room-hint">Trap</span>
                                )}
                                {threatHints?.hasMonster && (
                                  <span className="maplab-viewer-rail-room-hint">Monster</span>
                                )}
                                {threatHints?.hasEncounter && (
                                  <span className="maplab-viewer-rail-room-hint">Encounter</span>
                                )}
                                {hasNpcHint && (
                                  <span className="maplab-viewer-rail-room-hint">
                                    {npcUnion.length === 1 ? "1 NPC" : `${npcUnion.length} NPCs`}
                                  </span>
                                )}
                              </span>
                            )}
            ...
            {offMapRooms.filter((room) =>
              matchesRoom(room.room_id, room.title ?? `Room ${room.room_id}`, "Off map"),
            ).length > 0 && (
                  ...
                  {offMapRooms
                    ...
                    .map((room) => (
                      <li
                        ...
                        className="maplab-viewer-rail-room-item"
                        role="option"
                        aria-selected={room.room_id === activeRoomId}
                      >
                        <button
                          type="button"
                          aria-pressed={room.room_id === activeRoomId}
                          onClick={() => selectRoom(room.room_id)}
                        >
                          <span className="maplab-viewer-rail-room-name">
                            {room.title ?? `Room ${room.room_id}`}
                          </span>
                          <span className="maplab-viewer-rail-room-number" aria-hidden="true">
                            Off map · #{room.room_id}
                          </span>
                        </button>
                      </li>
                    ))}
