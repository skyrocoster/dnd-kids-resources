...
export function FixturePropertiesForm({
  spec,
  values,
  onChange,
  layout,
  currentDungeonId,
}: FixturePropertiesFormProps) {
  ...
  return (
    <div className="maplab-field-row maplab-fixture-live-picker">
      <SelectField
        ...
        options={[
          { value: "", label: "No NPC" },
          ...npcs.map((npc) => ({ value: String(npc.id), label: npc.name })),
        ]}
        value={selected}
        onChange={(event) =>
          onChange(field.key, event.target.value === "" ? null : Number(event.target.value))
...
function DestinationPickerField({
  ...
  const [mode, setMode] = useState<"here" | "elsewhere">(
    isGatewayValue(value) ? "elsewhere" : "here",
  );
  ...
  return (
    <div
      className="maplab-field-row maplab-destination-picker"
      role="group"
      aria-label={field.label}
    >
      <div
        className="maplab-destination-mode"
        role="radiogroup"
...
function InDungeonDestinationPicker({
  ...
  const roomsOnPickerFloor = useMemo(() => roomsOnZ(layout, pickerZ), [layout, pickerZ]);

  const destinationRoomId = useMemo(() => {
    if (!destination || destination.z !== pickerZ) return "";
    const room = roomsOnPickerFloor.find((candidate) =>
      absoluteCells(candidate).some(
        ([x, y]) => x === destination.cell[0] && y === destination.cell[1],
      ),
    );
...
function GatewayDestinationPicker({
  ...
  const selectedDungeon = isGatewayValue(value)
    ? dungeons.find((dungeon) => dungeon.id === value.dungeon_id)
    : undefined;
...
function LootBundlePickerField({
  ...
  return (
    <div className="maplab-field-row maplab-loot-bundle-picker">
      ...
      />
      {status === "error" && (
        <span className="maplab-loot-bundle-picker-status" role="status">
          Unable to load loot bundles.
        </span>
      )}
