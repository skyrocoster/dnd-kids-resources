...
import type { MapCell, MapDoor, MapFeature, MapLayout, MapPortal, MapProp, MapRoom, MapStair } from '../../../model/maplabModel'
...
type MapLabEditorSelectionProps = {
  ...
  authoredInspectorAdapter: (entity: MapDoor | MapProp | MapStair | MapPortal, kind: FixtureKind, id: number, update: MapLabEditorSelectionProps['updateFixtureFlags']) => ObstacleInspectorAdapter
    ...
    >
      ...
      <div className="maplab-selection-sheet-content" id="maplab-selection-sheet-content">
        {selectedFeature ? (
          ...
          <>
            <InspectorPanel target={{ kind: 'door', door: selectedDoor }} adapter={authoredInspectorAdapter(selectedDoor, 'door', selectedDoor.door_id, updateFixtureFlags)} />
            <FixturePropertiesForm spec={FIXTURE_TYPES.door} values={selectedDoor as unknown as Record<string, unknown>} onChange={(key, value) => updateFixtureFlags(selectedDoor.door_id, 'door', { [key]: value })} />
          </>
        ) : selectedRoom ? (
          <RoomContentEditor
          ...
          <>
            <InspectorPanel target={{ kind: 'prop', prop: selectedProp }} adapter={authoredInspectorAdapter(selectedProp, 'prop', selectedProp.prop_id, updateFixtureFlags)} />
            <FixturePropertiesForm
              spec={FIXTURE_TYPES.prop}
              values={{ ...selectedProp, side: selectedProp.side ?? 'Off' }}
              onChange={(key, value) => updateFixtureFlags(selectedProp.prop_id, 'prop', { [key]: key === 'side' && value === 'Off' ? undefined : value })}
            />
          </>
        ) : selectedStair ? (
          <>
            <InspectorPanel target={{ kind: 'stair', stair: selectedStair }} adapter={authoredInspectorAdapter(selectedStair, 'stair', selectedStair.stair_id, updateFixtureFlags)} />
            <div className="maplab-field-row maplab-stair-direction-row">
              <label htmlFor="maplab-stair-direction-up">{stairUpFloor !== null ? `Stairs up to floor ${stairUpFloor}` : 'Stairs up (no floor above)'}</label>
              <input id="maplab-stair-direction-up" type="checkbox" disabled={stairUpFloor === null || !selectedStairCell} checked={hasStairInDirection('up')} onChange={(event) => selectedStairCell && setStairDirection(activeZ, selectedStairCell, 'up', event.target.checked)} />
            </div>
            <div className="maplab-field-row maplab-stair-direction-row">
              <label htmlFor="maplab-stair-direction-down">{stairDownFloor !== null ? `Stairs down to floor ${stairDownFloor}` : 'Stairs down (no floor below)'}</label>
              <input id="maplab-stair-direction-down" type="checkbox" disabled={stairDownFloor === null || !selectedStairCell} checked={hasStairInDirection('down')} onChange={(event) => selectedStairCell && setStairDirection(activeZ, selectedStairCell, 'down', event.target.checked)} />
            </div>
            <FixturePropertiesForm spec={FIXTURE_TYPES.stair} values={selectedStair as unknown as Record<string, unknown>} onChange={(key, value) => updateFixtureFlags(selectedStair.stair_id, 'stair', { [key]: value })} />
          </>
        ) : selectedPortal ? (
          <>
            <InspectorPanel target={{ kind: 'portal', portal: selectedPortal }} adapter={authoredInspectorAdapter(selectedPortal, 'portal', selectedPortal.portal_id, updateFixtureFlags)} />
           <FixturePropertiesForm spec={FIXTURE_TYPES.portal} values={selectedPortal as unknown as Record<string, unknown>} layout={layout} currentDungeonId={routeDungeonId} onChange={(key, value) => updateFixtureFlags(selectedPortal.portal_id, 'portal', { [key]: value })} />
          </>
        ) : null}
      </div>
    </aside>
