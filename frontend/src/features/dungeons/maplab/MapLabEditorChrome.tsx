...
import {
  ChevronDownIcon, ChevronUpIcon, DoorClosedIcon, EraserIcon, EyeIcon, LinkIcon, MapIcon, MousePointer2,
  PlusIcon, PortalIcon, PropIcon, RoomIcon, SaveIcon, StairsIcon, Trees, Waves,
...
export function MapLabEditorChrome(props: MapLabEditorChromeProps) {
  const { state, floors, hasFloorAbove, hasFloorBelow, addFloorAbove, addFloorBelow, armedTool, setArmedTool, setPlacementError, brushArmed, eraseArmed,
    ...
    activatePropKind, activateTerrainTool, activateTopFilteredTool, flyoutFilter, setFlyoutFilter,
    dungeons, incomingGateways, connectionsLoaded, connectionsLoadError, selectPortal, setGatewayToRemove,
     handleAddReturnGateway } = props
  const dungeonIds = new Set(dungeons.map((dungeon) => dungeon.id))
  const resolutionCount = state.layout.portals.filter((portal) => portal.to === undefined).length
    + (connectionsLoaded ? state.layout.portals.filter((portal) => portal.to?.dungeon_id !== undefined && !dungeonIds.has(portal.to.dungeon_id)).length : 0)
    + incomingGateways.filter((gateway) => !state.layout.portals.some((portal) => portal.to?.dungeon_id === gateway.dungeon_id)).length
  ...
  const toolMenu = (
    ...
    >
      ...
      >
        {openFlyout === kind ? <ChevronUpIcon width={14} height={14} aria-hidden="true" /> : <ChevronDownIcon width={14} height={14} aria-hidden="true" />}
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Positioner
          anchor={groupRef}
        ...
        >
          ...
          >
            {filter(label)}
            {options.length === 0 ? <p className="maplab-tool-palette-empty">No tools match that.</p> : options.map((option) => {
              const Icon = option.icon
              return <button key={option.key} type="button" role="menuitem" className="maplab-pill-button" data-active={option.active || undefined} onClick={() => choose(option.key)}><Icon width={16} height={16} aria-hidden="true" />{option.label}</button>
            })}
          </Popover.Popup>
        </Popover.Positioner>
      </Popover.Portal>
  ...
  return <>
    {props.statusSlot && createPortal(<span className="maplab-editor-save-status" data-status={props.saveStatus.status} role="status" aria-live="polite"><SaveIcon width={16} height={16} aria-hidden="true" />{statusLabel(props.saveStatus.status)}</span>, props.statusSlot)}
     <div className="maplab-toolbar">
       <div className="maplab-toolbar-row maplab-toolbar-row--commands"><ToolbarTray groupKey="editor-primary" label="Primary"><div className="maplab-tool-palette" role="group" aria-label="Primary drawing tools">
      ...
      <div className="maplab-view-popover-wrap"><Popover.Root open={props.viewPopoverOpen} closeOnEscape={false} onOpenChange={(open) => { props.setViewPopoverOpen(open); if (open) { props.setMapPopoverOpen(false); props.setOpenUtility(null) } }}><Popover.Trigger ref={props.viewTriggerRef} type="button" className="maplab-pill-button maplab-editor-toolbar-button" aria-haspopup="true"><EyeIcon width={18} height={18} aria-hidden="true" />View</Popover.Trigger><Popover.Portal><Popover.Positioner side="bottom" align="end"><Popover.Popup className="maplab-view-popover" role="menu" initialFocus={false} finalFocus={(closeType) => closeType === 'keyboard' ? props.viewTriggerRef : false}>{(['outside', 'props', 'passages', 'labels'] as const).map((layer) => <button key={layer} type="button" className="maplab-pill-button maplab-layer-toggle-button" aria-pressed={props.layerVisible[layer]} onClick={() => props.toggleLayer(layer)}>{layer[0].toUpperCase() + layer.slice(1)}</button>)}<button type="button" className="maplab-pill-button" aria-pressed={props.showGhostFloor} disabled={props.ghostZ === null} onClick={() => props.setShowGhostFloor((a) => !a)}>Ghost lower floor</button><button type="button" className="maplab-pill-button" aria-pressed={props.density === 'detailed'} onClick={() => props.setDensity('detailed')}>Detailed</button><button type="button" className="maplab-pill-button" aria-pressed={props.density === 'auto'} onClick={() => props.setDensity('auto')}>Auto</button><button type="button" className="maplab-pill-button" aria-pressed={props.density === 'simple'} onClick={() => props.setDensity('simple')}>Simple</button></Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root></div>
      <div className="maplab-map-popover-wrap"><Popover.Root open={props.mapPopoverOpen} closeOnEscape={false} onOpenChange={(open) => { props.setMapPopoverOpen(open); if (open) { props.setViewPopoverOpen(false); props.setOpenUtility(null) } }}><Popover.Trigger ref={props.mapTriggerRef} type="button" className="maplab-pill-button maplab-editor-toolbar-button" aria-haspopup="true"><MapIcon width={18} height={18} aria-hidden="true" />Map</Popover.Trigger><Popover.Portal><Popover.Positioner side="bottom" align="end"><Popover.Popup className="maplab-map-popover" role="menu" initialFocus={false} finalFocus={(closeType) => closeType === 'keyboard' ? props.mapTriggerRef : false}>{(['top', 'right', 'bottom', 'left'] as const).map((side) => <label key={side} className="maplab-field-row maplab-room-content-field maplab-editor-padding-input"><span>{side[0].toUpperCase() + side.slice(1)}</span><input type="number" min={0} value={state.layout.meta.padding[side]} onChange={(e) => props.updatePadding({ ...state.layout.meta.padding, [side]: Number(e.target.value) })} /></label>)}<button type="button" className="maplab-pill-button maplab-editor-toolbar-button" onClick={() => props.setConfirmingReset(true)}>Reset unsaved changes</button></Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root></div>
       </div>
       <div className="maplab-toolbar-row maplab-toolbar-row--navigation">
        <div className="maplab-floor-tabs" role="tablist" aria-label="Dungeon floors">{floors.map((floor) => <button key={floor.z} type="button" role="tab" className="maplab-pill-button maplab-floor-tab" aria-selected={floor.z === state.activeZ} onClick={() => props.setActiveZ(floor.z)}>{floor.title ?? `Floor ${floor.z}`}</button>)}</div>
         <div className="maplab-editor-floor-actions" aria-label="Floor actions"><button type="button" className="maplab-pill-button maplab-editor-floor-action" disabled={hasFloorAbove} onClick={addFloorAbove}><PlusIcon width={16} height={16} aria-hidden="true" />Add floor above</button><button type="button" className="maplab-pill-button maplab-editor-floor-action" disabled={hasFloorBelow} onClick={addFloorBelow}><PlusIcon width={16} height={16} aria-hidden="true" />Add floor below</button></div>
         <button type="button" className="maplab-pill-button maplab-editor-room-list-new" onClick={props.onNewRoom}><PlusIcon width={16} height={16} aria-hidden="true" />New room</button>
          <ViewerRoomRail layout={state.layout} parsed={props.parsed} activeRoomId={state.selectedRoomId} onSelectRoom={props.onSelectRoom} open={props.openUtility === 'finder'} onOpenChange={(open) => { props.setOpenUtility(open ? 'finder' : null); if (open) { props.setViewPopoverOpen(false); props.setMapPopoverOpen(false) } }} />
          <div className="maplab-connections-utility">
            <Popover.Root open={props.openUtility === 'connections'} closeOnEscape={false} onOpenChange={(open) => { props.setOpenUtility(open ? 'connections' : null); if (open) { props.setViewPopoverOpen(false); props.setMapPopoverOpen(false) } }}>
              <Popover.Trigger ref={props.connectionsTriggerRef} type="button" className="maplab-pill-button" aria-haspopup="dialog" aria-expanded={props.openUtility === 'connections'}><LinkIcon width={18} height={18} aria-hidden="true" />Connections ({resolutionCount})</Popover.Trigger>
              <Popover.Portal>
                <Popover.Positioner side="bottom" align="end">
                  <Popover.Popup className="maplab-connections-panel" role="dialog" aria-label="Connection utilities" initialFocus={false} finalFocus={(closeType) => closeType === 'keyboard' ? props.connectionsTriggerRef : false}>
                    <ConnectionsResolveList layout={state.layout} dungeons={dungeons} incomingGateways={incomingGateways} connectionsLoaded={connectionsLoaded} connectionsLoadError={connectionsLoadError} onResolve={(portal) => { props.setActiveZ(portal.z); selectPortal(portal.portal_id); props.setOpenUtility(null); props.connectionsTriggerRef.current?.focus() }} onRemoveGateway={setGatewayToRemove} onAddReturnGateway={handleAddReturnGateway} />
                  </Popover.Popup>
                </Popover.Positioner>
              </Popover.Portal>
            </Popover.Root>
