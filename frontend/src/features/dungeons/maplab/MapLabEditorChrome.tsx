       ...
       <div className="maplab-toolbar-row maplab-toolbar-row--navigation">
        <div className="maplab-floor-tabs" role="tablist" aria-label="Dungeon floors">{floors.map((floor) => <button key={floor.z} type="button" role="tab" className="maplab-pill-button maplab-floor-tab" aria-selected={floor.z === state.activeZ} onClick={() => props.setActiveZ(floor.z)}>{floor.title ?? `Floor ${floor.z}`}</button>)}</div>
         ...
         <div className="maplab-connections-utility">
           <button ref={connectionsTriggerRef} type="button" className="maplab-pill-button" aria-haspopup="dialog" aria-expanded={props.openUtility === 'connections'} onClick={() => { props.setOpenUtility((open) => open === 'connections' ? null : 'connections'); props.setViewPopoverOpen(false); props.setMapPopoverOpen(false) }}><LinkIcon width={18} height={18} aria-hidden="true" />Connections ({resolutionCount})</button>
           {props.openUtility === 'connections' && <div className="maplab-connections-panel" role="dialog" aria-label="Connection utilities" onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); props.setOpenUtility(null); connectionsTriggerRef.current?.focus() } }}><ConnectionsResolveList layout={state.layout} dungeons={dungeons} incomingGateways={incomingGateways} connectionsLoaded={connectionsLoaded} connectionsLoadError={connectionsLoadError} onResolve={(portal) => { props.setActiveZ(portal.z); selectPortal(portal.portal_id); props.setOpenUtility(null); connectionsTriggerRef.current?.focus() }} onRemoveGateway={setGatewayToRemove} onAddReturnGateway={handleAddReturnGateway} /></div>}
         </div>
       </div>
    </div>
  </>
