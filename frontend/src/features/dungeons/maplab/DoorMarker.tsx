...
export function DoorMarker({
  ...
  const openLeafSegment = {
    x1: swing.hinge.x,
    y1: swing.hinge.y,
    x2: swing.leafTip.x,
    y2: swing.leafTip.y,
  };
  const renderedOpenLeafSegment = isConcealed
    ? insetSegment(openLeafSegment, HIDDEN_DOT_ENDPOINT_INSET)
    : openLeafSegment;
  const renderedClosedLeafSegment = isConcealed
    ? insetSegment(segment, HIDDEN_DOT_ENDPOINT_INSET)
    : segment;
