...
function replaceDungeonRoom(
  data: DungeonData,
  roomId: number,
  updater: (room: DungeonRoom) => DungeonRoom,
): DungeonData {
...
export function useMapLabEditor(dungeonId: number | null, initialDungeon: Dungeon | null = null) {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      if (shouldSaveLayout) {
        ...
        saveDungeonLayout(dungeonId, {
          data: stateRef.current.layout as unknown as Record<string, unknown>,
        })
      ...
      if (shouldSaveData) {
        ...
        updateDungeon(dungeonId, {
          ...
          .catch(() =>
            setDataSyncStatus({ status: "error", error: "Failed to save dungeon data" }),
          );
      }
    }, SAVE_DEBOUNCE_MS);
  }, [dungeonId]);
  ...
  useEffect(() => {
    ...
    const survivors = stateRef.current.layout.rooms.filter(
      (room) => !ghostIds.includes(room.room_id),
    );
  ...
  const selectRoom = useCallback(
    (roomId: number | null) => dispatch({ type: "selectRoom", roomId }),
    [],
  );
  ...
  const selectDoor = useCallback(
    (doorId: number | null) => dispatch({ type: "selectDoor", doorId }),
    [],
  );
  ...
  const deleteDoor = useCallback(
    (doorId: number) => apply({ type: "deleteDoor", doorId }),
    [apply],
  );

  const addProp = useCallback(
    (cell: MapCell, kind?: string) => apply({ type: "addProp", cell, kind }),
    [apply],
  );
  const selectProp = useCallback(
    (propId: number | null) => dispatch({ type: "selectProp", propId }),
    [],
  );
  const deleteProp = useCallback(
    (propId: number) => apply({ type: "deleteProp", propId }),
    [apply],
  );
  ...
  const selectStair = useCallback(
    (stairId: number | null) => dispatch({ type: "selectStair", stairId }),
    [],
  );
  const deleteStair = useCallback(
    (stairId: number) => apply({ type: "deleteStair", stairId }),
    [apply],
  );
  ...
  const selectPortal = useCallback(
    (portalId: number | null) => dispatch({ type: "selectPortal", portalId }),
    [],
  );
  const deletePortal = useCallback(
    (portalId: number) => apply({ type: "deletePortal", portalId }),
    [apply],
  );
  ...
  const selectFeature = useCallback(
    (featureId: number | null) => dispatch({ type: "selectFeature", featureId }),
    [],
  );
  const deleteFeature = useCallback(
    (featureId: number) => apply({ type: "deleteFeature", featureId }),
    [apply],
  );
  const updateFeatureMeta = useCallback(
    (featureId: number, meta: { title?: string; kind?: string }) =>
      apply({ type: "setFeatureMeta", featureId, meta }),
    [apply],
  );
