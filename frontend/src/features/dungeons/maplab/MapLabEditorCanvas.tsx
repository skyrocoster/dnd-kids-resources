...
export function MapLabEditorCanvas({
  ...
  selectPortal,
  selectProp,
  selectRoomContextually,
  selectDoorContextually,
  selectStairContextually,
  selectPortalContextually,
  ...
  const CELL_SIZE = 64;
  const contextualSelectionAllowed =
    !drawFeatureKind &&
    !placeDoorMode &&
    !placePropMode &&
    !placeStairMode &&
  ...
  return (
    ...
    >
      ...
      {layerVisible.outside &&
        featuresOnActiveFloor.map((feature) => (
          <g
            key={feature.feature_id}
            className="maplab-feature"
            data-feature-kind={feature.kind}
            data-maplab-target-kind="feature"
            data-maplab-target-id={feature.feature_id}
            ...
            onClick={() =>
              selectFeature(
                feature.feature_id === state.selectedFeatureId ? null : feature.feature_id,
              )
            }
            onKeyDown={(event) => {
      ...
      {roomsOnActiveFloor
        .filter((room) => !roomIsOffMap(room))
        .map((room) => (
          <g
            key={room.room_id}
            className="maplab-room"
            data-maplab-target-kind="room"
            data-maplab-target-id={room.room_id}
            ...
            onContextMenu={(event) =>
              handleContextMenu(event, () => selectRoomContextually(room.room_id))
            }
            onKeyDownCapture={(event) =>
              handleContextualKeyDown(event, () => selectRoomContextually(room.room_id))
            }
            onClick={() => {
              const nextRoomId = room.room_id === state.selectedRoomId ? null : room.room_id;
              selectRoom(nextRoomId);
              if (nextRoomId !== null) setArmedTool("select");
            }}
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") {
                event.preventDefault();
        ...
        <g className="maplab-room-brush-preview" aria-hidden="true">
          {strokeCells.map((cell) => (
            <rect
              ...
              data-brush-state={brushCellStateForCell(
                state.layout,
                state.activeZ,
      ...
      {layerVisible.passages &&
        doorsOnActiveFloor.map((door) => {
          ...
          return (
            <g
              key={door.door_id}
              data-maplab-target-kind="door"
              data-maplab-target-id={door.door_id}
              onContextMenu={(event) =>
                handleContextMenu(event, () => selectDoorContextually(door.door_id))
              }
              onKeyDownCapture={(event) =>
                handleContextualKeyDown(event, () => selectDoorContextually(door.door_id))
              }
            >
              <DoorMarker
                door={door}
      ...
      {layerVisible.passages && (
        <g className="maplab-door-badge-layer" aria-hidden="true">
          {doorsOnActiveFloor.map((door) => (
            <DoorBadgeLayer key={door.door_id} door={door} cellSize={CELL_SIZE} />
          ))}
        </g>
      )}
      {layerVisible.passages &&
        stairsOnActiveFloor.map((stair) => {
          ...
          if (!cell) return null;
          const { dx, dy, grouped } = markerOffset(
            state.layout,
            state.activeZ,
            cell,
            "stair",
          ...
          return (
            <g
              key={stair.stair_id}
              data-maplab-target-kind="stair"
              data-maplab-target-id={stair.stair_id}
              onContextMenu={(event) =>
                handleContextMenu(event, () => selectStairContextually(stair.stair_id))
              }
              onKeyDownCapture={(event) =>
                handleContextualKeyDown(event, () => selectStairContextually(stair.stair_id))
              }
            >
              <StairMarker
                stair={stair}
      ...
      {layerVisible.passages &&
        portalsOnActiveFloor.map((portal) => {
          const { grouped, ...offset } = markerOffset(
            state.layout,
            state.activeZ,
            portal.cell,
            "portal",
          ...
          return (
            <g
              ...
              data-maplab-target-kind="portal"
              data-maplab-target-id={portal.portal_id}
              onContextMenu={(event) =>
                handleContextMenu(event, () => selectPortalContextually(portal.portal_id))
              }
              onKeyDownCapture={(event) =>
                handleContextualKeyDown(event, () => selectPortalContextually(portal.portal_id))
              }
            >
              <PortalMarker
                portal={portal}
                ...
                onClick={() =>
                  selectPortal(
                    portal.portal_id === state.selectedPortalId ? null : portal.portal_id,
                  )
                }
              />
            </g>
              ...
              <rect
                ...
                onClick={(event) => {
                  ...
                  if (cellIsFull(state.layout, state.activeZ, [x, y])) {
                    setPlacementError(
                      "That square already has 4 markers — pick a different square.",
                    );
                    return;
                  }
      ...
      {layerVisible.props &&
        propsOnActiveFloor.map((prop) => {
          const propOffset =
            prop.side === undefined
              ? markerOffset(state.layout, state.activeZ, prop.cell, "prop", prop.prop_id)
              : undefined;
          return (
            <g
              key={prop.prop_id}
              data-maplab-target-kind="prop"
              data-maplab-target-id={prop.prop_id}
              onContextMenu={(event) =>
                handleContextMenu(event, () => selectPropContextually(prop.prop_id))
              }
              onKeyDownCapture={(event) =>
                handleContextualKeyDown(event, () => selectPropContextually(prop.prop_id))
              }
            >
              <PropMarker
                prop={prop}
                ...
                simplified={simplified}
                onClick={() =>
                  selectProp(prop.prop_id === state.selectedPropId ? null : prop.prop_id)
                }
              />
            </g>
          );
