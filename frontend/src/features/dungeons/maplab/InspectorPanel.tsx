...
import { fixtureInspectableDescriptor, inspectableDescriptor, type FixtureStateChip } from './maplabPresentation'
import {
  defaultFixtureState,
  ...
  return (
    <section className="maplab-inspector-state" aria-label={heading}>
      ...
      <div className="maplab-inspector-obstacle-grid">
        ...
        />
        <span className="maplab-inspector-shown-dash" aria-hidden="true">—</span>
      ...
      {dcLines.length > 0 && (
        <div className="maplab-inspector-dc-lines">
          {dcLines.map((dc) => (
            <p key={dc.obstacle} className="maplab-inspector-obstacle-dc">{dc.text}</p>
          ))}
        </div>
      )}
    </section>
...
export function InspectorPanel({
  ...
  const descriptor = isFixture ? fixtureInspectableDescriptor(target, context) : inspectableDescriptor(target, context)
  ...
  switch (target.kind) {
    case 'door':
      fixture = {
        effective: effectiveFixtureState(target.door.state ?? defaultFixtureState(), target.session),
        flags: target.door,
        isDoor: true,
      }
      break
    case 'stair':
      fixture = {
        effective: effectiveFixtureState(target.stair.state ?? defaultFixtureState(), target.session),
        flags: target.stair,
        isDoor: false,
      }
      break
    case 'portal':
      fixture = {
        effective: effectiveFixtureState(target.portal.state ?? defaultFixtureState(), target.session),
        flags: target.portal,
        isDoor: false,
      }
      break
    case 'prop':
      fixture = {
        effective: effectiveFixtureState(target.prop.state ?? defaultFixtureState(), target.session),
        flags: target.prop,
        isDoor: false,
      }
      break
  ...
  const dcLines: { obstacle: ObstacleKey; text: string }[] = fixture
    ? [
        ...(fixture.effective.obstacles.concealment.armed
          ? [{ obstacle: 'concealment' as const, text: dcSegment('Perception', fixture.flags.hiddenDc) }]
          : []),
        ...(fixture.effective.obstacles.lock.armed
          ? [
              {
                obstacle: 'lock' as const,
                text: [dcSegment('Break', fixture.flags.breakDc), dcSegment('Pick', fixture.flags.pickDc)].join(' · '),
              },
            ]
          : []),
  ...
  const metaLines = descriptor.lines.filter(
    (line) => !['Position', 'Break DC', 'Pick DC', 'Perception DC', 'Search DC'].includes(line.label),
  )
  ...
  const armedDisabled = (obstacle: ObstacleKey) =>
    obstacle === 'concealment'
      ...
      : !(adapter?.onToggleArmed || (obstacle === 'lock' ? controls?.onToggleLocked : controls?.onDisarmTrap))
      ...
      {descriptor.chips.length > 0 && (
        <div className="maplab-inspector-chips">
          {descriptor.chips.map((chip) => {
            ...
            const token = FIXTURE_CHIP_TOKENS[chip.state as FixtureStateChip['state']] ?? '--md-on-surface-variant'
            return (
              <span
                key={chip.state}
