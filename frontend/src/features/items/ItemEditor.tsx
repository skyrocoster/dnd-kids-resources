...
export function ItemEditor({ item, onClose, onSaved }: ItemEditorProps) {
  ...
  const [form, setForm] = useState<ItemFormState>(() =>
    item ? itemToFormState(item) : emptyItemForm(),
  );
  ...
  const patch = (fields: Partial<ItemFormState>) =>
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const saved = item
        ? await api.updateItem(item.id, formStateToItemInput(form))
        : await api.createItem(formStateToItemInput(form));
  ...
  return (
    ...
    >
      {status.message && (
        <p role="status" className={`item-editor-status ${status.kind || ""}`}>
          {status.message}
        </p>
      )}
      <form id={formId} onSubmit={handleSubmit} className="item-editor-form">
        <div className="item-editor-grid">
          ...
          <TextField
            ...
            step="any"
            value={form.valueGp}
            onChange={(event) => patch({ valueGp: event.target.value })}
            error={errors.valueGp}
            required
          />
          <SelectField
            label="Category"
            ...
            options={ITEM_CATEGORIES.map(({ slug, label }) => ({ value: slug, label }))}
          />
        </div>
        <TextField
          label="Description"
