...
export function WeaponEditor({ weapon, draftWeapon, onClose, onSaved }: WeaponEditorProps) {
  ...
  const [form, setForm] = useState<WeaponFormState>(() =>
    weapon
      ? weaponToFormState(weapon)
      : draftWeapon
        ? weaponToFormState(draftWeapon)
        : emptyWeaponForm(),
  );
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const saved = weapon
        ? await api.updateWeapon(weapon.id, payload)
        : await api.createWeapon(payload);
      setStatus({ message: "Weapon saved.", kind: "success" });
      onSaved(saved);
    } catch (error) {
      setStatus({
        message: error instanceof Error ? error.message : "Failed to save weapon.",
        kind: "error",
      });
      setSaving(false);
    }
    ...
    >
      ...
      <form id={formId} onSubmit={handleSubmit} className="weapon-editor-form">
        ...
        <section className="weapon-editor-section">
          ...
          {form.attackRows.length === 0 && (
            <p className="weapon-editor-empty">No attack rows added.</p>
          )}
