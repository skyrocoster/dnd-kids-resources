...
describe("grid rendering", () => {
  ...
  it("marks played vs planned styling per lane", () => {
    ...
    const played = lane1.ordered.filter((n) => n.kind === "session" || n.fulfilled_at);
    const planned = lane1.ordered.filter((n) => n.kind === "beat" && !n.fulfilled_at);
  ...
  it("exposes Planned beats in header and Current position in divider", () => {
    const tapestry = demoTapestry();
    const { container: swimContainer } = render(
      <LoomSwimlanes threads={tapestry.threads} nodes={tapestry.nodes} sessions={sessions} />,
    );
    ...
    const thread = tapestry.threads[0];
    const { container: laneContainer } = render(
      <LoomLane thread={thread} nodes={tapestry.nodes} sessions={sessions} />,
    );
  ...
  it("does not invoke callbacks for unknown action type", () => {
    ...
    const gaps = document.querySelectorAll(".loom-lane-gap");
    const dt = {
      getData: () => JSON.stringify({ action: "unknown" }),
      setData: vi.fn(),
    } as unknown as DataTransfer;
    fireEvent.drop(gaps[0], { dataTransfer: dt });
