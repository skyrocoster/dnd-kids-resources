...
describe("LoomNodeEditor", () => {
  ...
  it("creates a session node", async () => {
    ...
    render(<LoomNodeEditor onClose={() => {}} onSaved={onSaved} />);
  ...
  it("creates a beat node with the beat kind selected", async () => {
    ...
    const createLoomNode = vi.spyOn(api, "createLoomNode").mockResolvedValue(created);
    const user = userEvent.setup();
    render(<LoomNodeEditor initialKind="beat" onClose={() => {}} onSaved={() => {}} />);
  ...
  it("edits an existing node, locking kind", async () => {
    ...
    const user = userEvent.setup();
    render(<LoomNodeEditor node={node} onClose={() => {}} onSaved={() => {}} />);

    expect(
      screen.getByRole("dialog", { name: "Edit Session: Tracks lead to the cave" }),
    ).toBeInTheDocument();
  ...
  it("shows a save error inline", async () => {
    ...
    const user = userEvent.setup();
    render(<LoomNodeEditor onClose={() => {}} onSaved={() => {}} />);
