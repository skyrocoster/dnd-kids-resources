...
describe("LoomPage", () => {
  ...
  it("renders the tapestry nodes and the beat bank tray once loaded", async () => {
    ...
    expect(
      screen.queryByText("Track where every story thread stands between sessions."),
  ...
  it("renders Now and Next badges on the active thread", async () => {
    ...
    expect(screen.getByRole("button", { name: "beat: Confront the goblin chief" })).toHaveClass(
      "loom-node--ghosted",
    );
  });
  ...
  it("retries loading the tapestry from the load error state", async () => {
    const getTapestrySpy = vi
      .spyOn(api, "getLoomTapestry")
  ...
  it("shows the empty-tapestry state when there are no threads or nodes", async () => {
    ...
    expect(
      screen.getByText("Create a thread to start tracking campaign progress."),
    ).toBeInTheDocument();
  ...
  it("submits the session log with live thread outcomes and reloads", async () => {
    vi.spyOn(api, "getLoomTapestry").mockResolvedValue(demoTapestry());
    const logSpy = vi
      .spyOn(api, "logLoomSession")
      .mockResolvedValue({ id: 5, ordinal: 5, name: "Session 5", played_on: null, notes: null });
    const user = userEvent.setup();
    render(<LoomPage />);
    await waitFor(() => expect(screen.getAllByText("The Lost Puppy").length).toBeGreaterThan(0));
    await user.click(screen.getByRole("button", { name: "Advance Campaign" }));
  ...
  it("submits quiet outcomes for threads without a pending beat", async () => {
    ...
    const logSpy = vi
      .spyOn(api, "logLoomSession")
      .mockResolvedValue({ id: 2, ordinal: 2, name: "Session 2", played_on: null, notes: null });
    const user = userEvent.setup();
    render(<LoomPage />);
  ...
  it("calls moveLoomThreadItem on cross-lane drop", async () => {
    ...
    await waitFor(() => {
      expect(moveSpy).toHaveBeenCalledWith(1, 2, {
        target_thread_id: 2,
