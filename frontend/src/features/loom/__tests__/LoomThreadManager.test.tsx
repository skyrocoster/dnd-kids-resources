...
describe("LoomThreadManager", () => {
  ...
  it("moves the exclusive color selection with the arrow keys", async () => {
    ...
    expect(within(group).getByRole("radio", { name: "thread-2" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
  ...
  it("renames and recolors an existing thread", async () => {
    ...
    await waitFor(() =>
      expect(updateLoomThread).toHaveBeenCalledWith(
        1,
        expect.objectContaining({ name: "Renamed", color: "thread-4" }),
      ),
    );
  });
