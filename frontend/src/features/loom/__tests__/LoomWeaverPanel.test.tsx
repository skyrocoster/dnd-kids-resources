...
describe("LoomWeaverPanel", () => {
  it("renders a selected beat node's title and kind", () => {
    ...
    expect(
      screen.getByText("The goblin chief knows where the puppy ended up."),
    ).toBeInTheDocument();
  });
  ...
  it("shows spawn thread action for session nodes", async () => {
    ...
    render(
      <LoomWeaverPanel {...baseProps} selectedNode={sessionNode} onSpawnThread={onSpawnThread} />,
    );
