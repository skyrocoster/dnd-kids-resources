...
function ManageSpellPlayersDialog({ spell, onClose }: ManageSpellPlayersDialogProps) {
  const [playersRemote, setPlayersRemote] =
    useState<RemoteState<{ players: Player[]; assignedPlayers: Player[] }>>(initialRemoteState);
  ...
  const save = async () => {
    ...
    setActionError(null);
    try {
      const assignedPlayers = await api.replaceSpellPlayers(
        spell.id,
        Array.from(draftIds).sort((a, b) => a - b),
      );
      setDraftIds(new Set(assignedPlayers.map((player) => player.id)));
      onClose();
    } catch (error) {
    ...
    <Dialog
      ...
      footer={
        <>
          ...
          <Button type="button" onClick={save} loading={pending} disabled={loadFailed}>
            Save
          </Button>
        </>
      }
    >
      {playersRemote.status === "loading" || playersRemote.status === "idle" ? (
        ...
        <div className="spell-player-dialog-content">
          ...
          {actionError && (
            <p className="spell-player-action-error" role="status">
              {actionError}
            </p>
          )}
          {players.length === 0 ? (
      ...
      <BrowserLayout
        ...
        list={
          <SearchList
            ...
            status={
              spellsRemote.status === "loading" || spellsRemote.status === "idle"
                ? "loading"
                : spellsRemote.status === "error"
                  ? "error"
                  : "ready"
            }
          />
          ...
          selected ? (
            <div className="spell-browser-detail">
              ...
              <Card
                ...
                tag={
                  selected.concentration ? "Concentration" : selected.ritual ? "Ritual" : undefined
                }
                variant="spell"
                footer={
                  <div className="spell-browser-actions">
                    <Button variant="secondary" onClick={() => setManagingPlayersSpell(selected)}>
                      Manage Players
                    </Button>
              ...
              >
                ...
                {selected.alternate_description && (
                  <p>
            ...
            <StatePanel
              status="noSelection"
              message="Choose a spell from the list to view its details."
            />
          )
        }
        editor={
