...
export function SpellEditor({ spell, onClose, onSaved }: SpellEditorProps) {
  ...
  const [form, setForm] = useState<SpellFormState>(() =>
    spell ? spellToFormState(spell) : emptySpellForm(),
  );
  ...
  useEffect(() => {
    ...
    api
      .getAbilities()
      .then((abilities) =>
        setAbilityOptions(abilities.map((a) => ({ value: a.code, label: a.name }))),
      )
      .catch(() => setAbilityOptions([]));
    api
      .getSpellComponents()
      .then((components) =>
        setComponentOptions(components.map((c) => ({ value: c.code, label: c.name }))),
      )
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const saved = spell
        ? await api.updateSpell(spell.id, payload)
        : await api.createSpell(payload);
      setStatus({ message: "Spell saved.", kind: "success" });
      onSaved(saved);
    } catch (error) {
      setStatus({
        message: error instanceof Error ? error.message : "Failed to save spell.",
        kind: "error",
      });
      setSaving(false);
    }
      ...
      <form id={formId} onSubmit={handleSubmit} className="spell-editor-form" noValidate>
            ...
            <div className="spell-editor-row-card" key={row.id}>
              <div className="spell-editor-row-grid">
                <SelectField
                  ...
                  value={row.kind}
                  onChange={(e) =>
                    updateAttackRow(row.id, { kind: e.target.value as AttackRow["kind"] })
                  }
                  options={[
                    ...
                    <CheckboxField
                      ...
                      onChange={() =>
                        updateAttackRow(row.id, {
                          savingThrows: toggleRowMulti(row.savingThrows, ability.value),
                        })
                      }
                    />
                  ))}
                    ...
                    <CheckboxField
                      ...
                      onChange={() =>
                        updateDamageRow(row.id, {
                          damageTypes: toggleRowMulti(row.damageTypes, dt.value),
                        })
                      }
                    />
                  ))}
        ...
        <section className="spell-editor-section">
          ...
          <div className="spell-editor-inline-grid">
            <DiceRollField
              label="Amount"
              value={form.healingAmount}
              onChange={(healingAmount) => patch({ healingAmount })}
            />
          </div>
