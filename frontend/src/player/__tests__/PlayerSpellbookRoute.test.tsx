...
describe('PlayerSpellbookRoute character choices', () => {
  ...
  it('retains the selected character when the route child remounts under the same session provider', async () => {
    ...
    expect(within(group).getByRole('button', { name: 'Ranger' })).toHaveAttribute('aria-pressed', 'true')
    ...
    expect(within(remountedGroup).getByRole('button', { name: 'Ranger' })).toHaveAttribute('aria-pressed', 'true')
    expect(within(remountedGroup).getByRole('button', { name: 'Wizard' })).toHaveAttribute('aria-pressed', 'false')
  })
})
