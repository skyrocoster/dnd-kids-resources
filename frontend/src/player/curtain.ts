...
type KidProp = Pick<MapProp, 'prop_id' | 'kind' | 'cell' | 'side' | 'z' | 'title'> & KidFixtureFacts
...
export function playerViewTransform(
  ...
  const fixture = <T extends { hidden?: boolean; locked?: boolean; trapped?: boolean; state?: unknown }>(
    value: T,
    session: SessionFixtureState | PassageSessionState | undefined,
  ) => {
    const sessionValue = session as (SessionFixtureState & { hidden?: boolean; locked?: boolean; trapped?: boolean; isOpen?: boolean }) | undefined
    const authored = (value.state ?? fixtureStateFromFlags(value as Parameters<typeof fixtureStateFromFlags>[0])) as FixtureState
  ...
  return {
    ...
    doors: layout.doors
      ...
      .map(({ d, effective }) => ({ door_id: d.door_id, cell: d.cell, side: d.side, z: d.z, title: d.title, ...facts(effective) })),
    stairs: layout.stairs
      .map(s => ({ s, effective: fixture(s, sessionFor('stairs', s.stair_id)) }))
      .filter(({ effective }) => effective.visible)
      .map(({ s, effective }) => ({ stair_id: s.stair_id, from: s.from, to: s.to, title: s.title, ...facts(effective) })),
    floors: layout.floors,
    props: layout.props
      .filter(p => p.kind !== 'encounter')
      .map(p => ({ p, effective: fixture(p, sessionFor('props', p.prop_id)) }))
      .filter(({ effective }) => effective.visible)
      .map(({ p, effective }) => ({ prop_id: p.prop_id, kind: p.kind, cell: p.cell, side: p.side, z: p.z, title: p.title, ...facts(effective) })),
    portals: layout.portals
      .map(p => ({ p, effective: fixture(p, sessionFor('portals', p.portal_id)) }))
      .filter(({ effective }) => effective.visible)
      .map(({ p, effective }) => ({ portal_id: p.portal_id, cell: p.cell, z: p.z, title: p.title, to: p.to, ...facts(effective) })),
  }
}
