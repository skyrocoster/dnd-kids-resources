...
function makeWheelEvent(
  overrides: Partial<WheelEvent> & { currentTarget?: EventTarget },
): WheelEvent {
  return {
    ctrlKey: false,
    metaKey: false,
...
function makePointerEvent(
  overrides: Partial<PointerEvent> & { target?: EventTarget },
): PointerEvent {
  return {
    clientX: 0,
    clientY: 0,
...
describe("useMapCanvasZoom", () => {
  ...
  it("fitToBounds calculates appropriate scale and pan", () => {
    ...
    act(() =>
      result.current.fitToBounds(
        { minX: 0, maxX: 9, minY: 0, maxY: 4 },
  ...
  it("in pan mode a drag starting on a .maplab-room element pans; in tool mode a single-pointer drag never pans", () => {
    ...
    act(() =>
      panResult.current.handlePointerDown(
        makePointerEvent({ clientX: 100, clientY: 100, target: roomEl }),
      ),
    );
    ...
    act(() =>
      toolResult.current.handlePointerDown(
        makePointerEvent({ clientX: 100, clientY: 100, target: plainTarget }),
      ),
    );
    act(() =>
      toolResult.current.handlePointerMove(makePointerEvent({ clientX: 130, clientY: 80 })),
    );
  ...
  it("two-pointer pinch zooms about the pinch centroid and hands back to drag-pan on release", () => {
    ...
    act(() =>
      result.current.handlePointerMove(
        makePointerEvent({ pointerId: 2, clientX: 300, clientY: 100 }),
      ),
    );
    ...
    act(() =>
      result.current.handlePointerMove(
        makePointerEvent({ pointerId: 1, clientX: 80, clientY: 130 }),
      ),
    );
    expect(result.current.zoom.scale).toBeCloseTo(2);
  ...
  it("pinch clamps to MAX_SCALE and starts even when a finger lands on an interactive target", () => {
    ...
    act(() =>
      result.current.handlePointerMove(
        makePointerEvent({ pointerId: 2, clientX: 1100, clientY: 100 }),
      ),
    );
  ...
  it("pointercancel/pointerup clears the gesture so a lifted finger leaves no phantom pan", () => {
    ...
    act(() =>
      result.current.handlePointerDown(
        makePointerEvent({ pointerId: 7, clientX: 100, clientY: 100, target: plainTarget }),
      ),
    );
    act(() =>
      result.current.handlePointerUp(
        makePointerEvent({ pointerId: 7, clientX: 100, clientY: 100 }),
      ),
    );
    act(() =>
      result.current.handlePointerMove(
        makePointerEvent({ pointerId: 7, clientX: 400, clientY: 400 }),
      ),
    );
