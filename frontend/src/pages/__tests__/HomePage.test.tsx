...
describe("HomePage", () => {
  ...
  it("shows the Reference chapter links by default", () => {
    ...
    expect(within(panel).getByRole("link", { name: "Monsters" })).toHaveAttribute(
      "href",
      "/monsters",
    );
    expect(within(panel).getByRole("link", { name: "Weapons" })).toHaveAttribute(
      "href",
      "/weapons",
    );
  });
  ...
  it("switches chapters when a tab is selected", async () => {
    ...
    expect(within(panel).getByRole("link", { name: "Loot Bundles" })).toHaveAttribute(
      "href",
      "/loot",
    );
  ...
  it("selects the next chapter and shows its links with the keyboard", async () => {
    ...
    expect(tab).toHaveAttribute("aria-controls", panel.id);
    expect(panel).toHaveAttribute("aria-labelledby", tab.id);
    expect(within(panel).getByRole("link", { name: "Players" })).toHaveAttribute(
      "href",
      "/players",
    );
    expect(screen.queryByRole("link", { name: "Spells" })).not.toBeInTheDocument();
