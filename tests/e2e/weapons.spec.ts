...
import type { Weapon } from "../../frontend/src/api/types";
...
test("edits a weapon and refreshes its displayed details", async ({ page }) => {
  ...
  await page.route("**/api/weapons/2", async (route) => {
    ...
    listedWeapons = listedWeapons.map((weapon) =>
      weapon.id === savedWeapon.id ? savedWeapon : weapon,
    );
...
test("creates a weapon and adds it to the browser list", async ({ page }) => {
  ...
  expect(createPayload).toMatchObject({
    name: "Ashen Staff",
    quick_rules: "A staff made from pale ash.",
  });
});
...
test("rejects an unknown quick-rule token before sending the create request", async ({ page }) => {
  ...
  await expect(
    page.getByRole("dialog", { name: "Add New Weapon" }).getByRole("status"),
  ).toContainText("Unknown token: unknown_bonus");
