...
planner reads in full and then hand-summarises. The measured cost of that is not small:
in the executor transcripts a single `npm run lint` result is ~1,129 tokens and
`npm run test:check` ~1,135, and reconcile pays for all five plus the transcription.

None of that reading changes a decision. What reconcile needs from each check is whether
it passed and — only when something failed — enough output to act on. So this script
runs all five and prints about ten lines.
