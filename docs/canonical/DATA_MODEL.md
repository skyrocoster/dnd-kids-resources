...
- **Dungeon room entries** — Retain authored array order and do not persist a separate entry ID. Their stable lookup identity is derived as `<room_id>:<one-based entry ordinal>`, preserving legacy dungeon JSON unchanged.
- **Passage-like map objects** — Doors, stairs, props, and portals share nested authored obstacle state. A fixture's authored `open` and obstacle `armed`/`shown` leaves live in `map_layout.data`; per-kind `next*Id` counters in layout metadata are monotonic and are never reduced after deletion.
- **`map_session_state`** — One row per dungeon holding sparse live overrides for doors, stairs, props, and portals plus the selected party room, kept separate from `map_layout` so play-state changes never dirty the authored map document. The backend retains only `open`, obstacle `armed`/`shown`, and non-null `partyRoomId`; absence falls back to authored state while explicit `false` persists. Missing or normalized-empty state is treated as empty; layout saves prune overrides invalidated by fixture deletion, geometry changes, or matching authored leaf changes; "Reset dungeon" deletes the row rather than writing empty state back.
...
| `map_layout` | `data` | Raw MapLayout blob: rooms, doors, stairs, floors, props, portals, fixtures, and per-kind monotonic next-ID counters in `meta`. Props may soft-reference a loot bundle by `bundle_id` with cached `bundle_name`; bundle contents resolve live. Props of kind `encounter` and `npc` likewise soft-reference `encounters.id` / `npcs.id` by `encounter_id` / `npc_id` — one-way, no foreign key and no cascade, so a deleted encounter or NPC leaves a dangling id that the UI degrades gracefully rather than a broken layout. A portal's `to` is optional (`{"z", "cell"}` for an in-dungeon pair, or `{"dungeon_id"}` for a cross-dungeon gateway) and is a one-way reference — no foreign key, no cascade, no reverse index; `GET /api/dungeons/{id}/incoming-gateways` scans other dungeons' blobs to surface the other side. The player curtain may read this raw API-visible blob, but its `KidMapLayout` output removes loot and authoring/concealment fields. | (JSON blob per dungeon) |
| `map_session_state` | `data` | Sparse session overrides for doors, stairs, props, and portals plus the selected room: `{"doors": {...}, "stairs": {...}, "props": {...}, "portals": {...}, "partyRoomId": 17}`. Only runtime `open` and obstacle `armed`/`shown` leaves are stored; absent leaves resolve from authored layout state. | (JSON blob per dungeon) |
