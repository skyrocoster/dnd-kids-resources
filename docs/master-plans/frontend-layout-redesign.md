...
| FL-08 | Plan | Implemented; awaiting human acceptance | [Map Lab Room Finder](../plans/done/maplab-room-finder/maplab-room-finder.md) replaces the editor and viewer's permanent room navigation with the shared `Find room…` finder; focused and full automated checks passed, and core live viewer behavior passed, but off-map live verification was unavailable. |
| Frontend layout recovery | Plan | Implemented; awaiting human acceptance | [Frontend Layout Recovery](../plans/done/frontend-layout-recovery/frontend-layout-recovery.md) restores the combined shallow shell, sequential constrained catalogs, compact Map Lab command grammar, bounded finder/utilities, and workspace scroll ownership across the required screenshot matrix; automated gates passed and acceptance evidence is archived under `artifacts/frontend-layout-audit-2026-08-12/`. |
