...
| FL-05 | Plan | Accepted 2026-08-09 | [Map Lab Shared Navigation](../plans/done/maplab-shared-navigation/maplab-shared-navigation.md) repaired shared editor/viewer centering, restoration, empty-space clearing, and viewer Escape behavior; focused and full automated checks passed, and the user accepted the visible outcome recorded in the [FL-05 acceptance script](../FL-05-maplab-navigation-acceptance.md). |
| FL-06 | Plan | Accepted 2026-08-09 | [Map Lab Command Band](../plans/done/maplab-command-band/maplab-command-band.md) shipped the editor and viewer command-band grammar, responsive rows, native keyboard semantics, and focused regression coverage; automated stage checks passed, and the user accepted the visible outcome. |
| FL-07 | Plan | Accepted 2026-08-11 | [Map Lab Relocations](../plans/done/maplab-relocations/maplab-relocations.md) moved floor creation to labelled command-band controls, selected-room deletion to the inspector, and connection resolution to a separate labelled utility; full checks passed and live acceptance passed at wide and constrained editor viewports with no console errors or relevant failed requests. |
| FL-08 | Plan | Accepted 2026-08-12 | [Map Lab Room Finder](../plans/done/maplab-room-finder/maplab-room-finder.md) replaces the editor and viewer's permanent room navigation with the shared `Find room…` finder; focused and full automated checks passed and core live viewer behavior passed, and after Frontend Layout Recovery the user accepted the visible result; off-map live verification remains unavailable. |
| Frontend layout recovery | Plan | Accepted 2026-08-12 | [Frontend Layout Recovery](../plans/done/frontend-layout-recovery/frontend-layout-recovery.md) restores the combined shallow shell, sequential constrained catalogs, compact Map Lab command grammar, bounded finder/utilities, and workspace scroll ownership across the required screenshot matrix; automated gates passed, acceptance evidence is archived under `artifacts/frontend-layout-audit-2026-08-12/`, and the user accepted the recovered presentation. |
| FL-09 | Plan | Accepted 2026-08-12 | [Map Lab Inspector Presentation](../plans/done/maplab-inspector-presentation/maplab-inspector-presentation.md) implements selection-only inspector presentation across editor and viewer, with wide docking and constrained overlay behavior while preserving target and inspector state; focused automated tests passed 42/42, Luna diagnostics found no console errors or confirmed failed requests but did not complete the full scripted matrix, and the user manually tested the explicit FL-09 expected-result checklist and accepted the result. |
