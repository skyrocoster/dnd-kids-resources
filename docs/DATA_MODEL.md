...
- **Passage-like map objects** — Doors, stairs, props, and portals share the knowledge-fact vocabulary `exists`, `lock`, and `trap`. `hiddenDc` is the Perception check to notice that an object exists; optional `searchDc` is the separate check to find something concealed within a visible object, and both remain DM-only data.
- **`map_session_state`** — One row per dungeon holding live door/stair/portal toggle overrides and the selected party room, kept separate from `map_layout` so play-state changes never dirty the authored map document. Missing session-state data is treated as empty (every fixture at its authored default and no party room selected); "Reset dungeon" deletes the row rather than writing empty state back.
