...
> **Status:** S0-S3, B0-B3, F0, and F1 shipped. Phase F in progress; F2 (player spell rewire) next.
...
- The current spell UI and editor share `frontend/src/api/types.ts::Spell`; the browser reads
  ...
  Their rewiring is the scope of Phase F.
...
- **Inherits:** F0 types, F1's shared form/browser mappings and constants, and B3 player-spell responses.
...
F2 - Player spell rewire: move player spell consumers and regression fixtures to target data.
