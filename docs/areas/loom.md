...
- **Beat order:** the focused-thread **Reorder Beats** modal reorders only upcoming beat nodes. It sends the position of the new follower (or a final-position sentinel) to the existing reorder endpoint, so its client-side list cannot place a beat before a session.
