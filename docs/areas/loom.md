...
> **Active plan:** [Loom Storyline Refactor](../plans/active/loom-storyline-refactor.md#pa2-beat-lifecycle-sharing-and-spawning-next-up).
...
- Backend (shipped): `backend/app/routers/loom.py` (thread CRUD with auto Start/End, beat/session node CRUD, ordered-membership insert/reorder/remove, tapestry read, position PATCH — no edges/bridge), loom Pydantic models in `backend/app/schemas.py`, tables in `scripts/init_database.py`. Migration script: `scripts/migrate_loom_v2.py` (idempotent, backs up + reports).
...
- The backend tapestry is refactored from a flat DAG into ordered linear Threads (PA0 DDL + PA1 API, both shipped): no `loom_edges`; a per-thread total order via `position` on `loom_node_threads`; `kind IN ('start','end','beat','session')`; provenance columns; `origin_node_id` on threads. `start`/`end` nodes are created only by `POST /loom/threads` and removed only by deleting the whole thread; a `beat` is thread-exclusive (one membership anywhere). Acyclicity machinery is gone — a per-thread total order is inherently acyclic, so there is nothing left to check.
...
- **Active: the [Loom Storyline Refactor](../plans/active/loom-storyline-refactor.md#pa2-beat-lifecycle-sharing-and-spawning-next-up) (PA2–PD0).** PA0 (DDL, seeds, migration) and PA1 (thread/order/node API) shipped. PA2 (beat lifecycle, sharing, spawn) is next up.
