...
> **Status:** Not next up. [Docs Restructure](docs-restructure.md) holds the Documentation Governance next-up slot, and this plan is unblocked once that plan completes — or sooner if a second table test is run before the format is formalised. Nothing here blocks running a session: the first record already exists at `../../table-tests/2026-07-23-player-app-skeleton-stage-6.md`, written ahead of its own contract deliberately. No stages shipped.
