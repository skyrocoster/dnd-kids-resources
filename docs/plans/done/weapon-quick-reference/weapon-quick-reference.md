...
| 2 | Weapon detail now offers Copy as New, opening the existing modal editor as a prefilled create draft. Saving the copied draft creates a normal independent catalog weapon, leaves the source unchanged, and selects the new weapon after reload. |
| 3 | Weapon detail renders quick rules through the shared reference-text renderer before attacks and long entries, resolving sheet-ready totals or falling back to readable phrases. Delete confirmation now calls a new `GET /weapons/{id}/players` endpoint and names every assigned character before the cascading delete. |
| 4 | Every one of the 218 seeded weapons now carries authored quick_rules with explicit dice wording and structured attack/damage facts, generated by `scripts/generate_weapon_quick_rules.py`. A round-trip test proves the seed survives init→seed→export with exact quick_rules strings, and the spell migration tests now separate the migration-transform contract (`TARGET_FIELDS`) from the canonical seed-row shape (`CANONICAL_SEED_FIELDS`, which includes quick_rules). |
