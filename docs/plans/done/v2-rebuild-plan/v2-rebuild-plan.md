...
- **Read trigger:** v2 rebuild history
...
**⚠️ Known backend gaps found while building Task 9 — fix these first, before or as the first step of building each page.** Task 9 discovered that `backend/tests/conftest.py` hand-writes its own DB schema rather than running `scripts/init_database.py`, and it had drifted badly from the real DB — so these routers "pass tests" today but are broken or incomplete against the actual dev database (`dnd_kids_resources.db`). Verify directly against that DB (not just pytest) before building UI on top of any of them — see `python -c "from fastapi.testclient import TestClient; from backend.app.main import app; TestClient(app).get(...)"` pattern used in Task 9, or run uvicorn and curl. Specifics per resource (checked 2026-07-10):
- **`GET /api/encounters` currently 500s on the real DB.** `backend/app/routers/encounters.py`'s `_parse_encounter_row` checks `encounter.get("units")` to JSON-parse it, but the SQL aliases the column `units AS creatures` — so the parse never fires and the raw JSON string fails `Encounter.creatures: Optional[List[str]]` response validation. Real `creatures` data is also richer than `List[str]`: each entry is a dict (`monster_id`, `original_name`, `name`, `hp_current`, `hp_max`, `ac`, `status`, `conditions`) — the schema field needs to be `Optional[List[Dict[str, Any]]]`, same class of fix as `Spell.damage`/`Monster.senses` in Task 9.
- **NPCs are almost entirely unimplemented.** `backend/app/routers/npcs.py` only ever selects/writes `id, name`; the real `npcs` table has ~13 more columns (`race`, `gender`, `background`, `size`, `stats`, `armor_class`, `hit_points`, `speed`, `saving_throws`, `skills`, `senses`, `languages`, `appearance`, `notes`) that are completely inaccessible via the API today, and the current `NPC`/`NPCCreate`/`NPCUpdate` Pydantic models (`role`, `description`, `notes`) don't even name-match those real columns. This needs a full schema + router rewrite, not just widening a `SELECT`.
- **Quests only expose `title`/`description`/`reward`.** `status` and `notes` are dead fields on the `Quest` schema (no matching DB columns — always `None`); the real `quests` table also has `objectives`, `details`, `quest_giver`, `dungeon_id`, `location` that aren't wired up at all. `reward` is returned as a raw JSON-encoded string (e.g. `'["5 gp", "a thank-you pie"]'`) instead of being parsed to a list.
- **Players silently drop `class`.** `SELECT id, name, class, level FROM players` returns a dict key of `class`, but the `Player` schema's field is `class_` (Python keyword) — with no alias, Pydantic just leaves it `None`, so every player's class is lost in the API response even though the DB has real values (e.g. `"Wizard"`). Needs `SELECT ... class AS class_ ...` or a Pydantic `alias`. (`total_spell_slots`/`current_spell_slots` columns exist on `players` too but are intentionally out of scope — those are the HP/spell-slot trackers this project explicitly dropped for v2, per `CLAUDE.md`.)
- **Dungeons already work correctly** — `GET /api/dungeons` round-trips the full nested seed data fine; no fix needed, and it's a good reference for how loose (`Dict[str, Any]`) a schema can be when the shape is inherently variable.
- When you fix a router, also fix `backend/tests/conftest.py`'s matching hand-written table (there are two near-duplicate copies of the schema in that file — both need the same fix) so pytest stops giving false confidence. Reuse the `parse_json_list()` helper in `backend/app/db.py` (added in Task 9) for any list-typed column that might have legacy non-JSON string data.
