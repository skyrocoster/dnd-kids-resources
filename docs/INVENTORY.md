...
| Document | Type | Authority | Status | Read trigger | Update trigger |
|---|---|---|---|---|---|
| [areas/design.md](areas/design.md) | Area guide | Canonical | Active plan | Shared UI, tokens, shell, or accessibility | Design ownership, source map, or active work changes |
| [areas/dungeons.md](areas/dungeons.md) | Area guide | Canonical | No active plan | Dungeon behavior | Dungeons ownership, source map, or active work changes |
...
| [Glossary term tooltips — hover/tap explanations for rule terms in rendered text](plans/active/glossary-term-tooltips/glossary-term-tooltips.md) | Plan | Working | Stages 1-4 shipped. | Glossary term tooltips, rule-term explanations in rendered text | A stage ships, or its scope or settled decisions change |
| [Kid Map Viewer — the tablet gets the DM's map, and colour a child can say out loud](plans/active/kid-map-viewer/kid-map-viewer.md) | Plan | Working | Stages 1-9 shipped, plus a 6R repair pass. | How the kid map draws rooms, doors, stairs, markers and names; the shared map canvas; the curtain's per-field visibility; the kid colour families and their solver; the party marker; room label placement in either app | A stage ships, or its scope or settled decisions change |
| [Player Map Knowledge — the DM controls each fact the party has learned](plans/active/player-map-knowledge/player-map-knowledge.md) | Plan | Working | Stages 1-3 shipped. | Reversible per-fact disclosure, the map knowledge document, inspector selection, or the DM's player-result preview | A stage ships, or its scope or settled decisions change |
...
| [Table Testing Records — real sessions become a comparable, checked record](plans/active/table-testing-records/table-testing-records.md) | Plan | Working | Stage 1 shipped — the format is documented in `docs/TABLE_TESTING.md` and routable, and both existing records conform to it. | Recording a real session, the table-test format, or its lifecycle and checks | A stage ships, or its scope or settled decisions change |
