...
| [Kid Spellbook — Give children a read-only assigned-spell reference](plans/active/kid-spellbook/kid-spellbook.md) | Plan | Working | Stages 1–3 shipped — the kid spellbook route, character bootstrap, sticky in-memory session state, and resilient polling are complete; Stage 4 browse modes and spell details are next. | When changing the kid spellbook route, assigned-spell payload, spell categories, or the DM spell editor's category assignment. | A stage ships, or its scope or settled decisions change |
