...
| [Map Lab test suite refactor — smaller, behavior-oriented test files](plans/active/maplab-test-suite-refactor/maplab-test-suite-refactor.md) | Plan | Working | Stage 2 shipped — MapLabPage is split into five behavior-oriented suites with all current viewer coverage preserved; ready for the Stage 3 editor/model split. | Splitting or reorganizing oversized Map Lab frontend tests without changing product behavior or reducing regression coverage. | A stage ships, or its scope or settled decisions change |
