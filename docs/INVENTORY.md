...
| [Map Obstacle State - authored baselines and one persisted run overlay](plans/active/map-obstacle-state/map-obstacle-state.md) | Plan | Working | Stage 5 shipped — shared map badge primitives now serve DM and player consumers, DM badges use armed-state policy, and the player map renders one icon-only Trap-or-Lock cue across all fixture kinds. | Map fixture concealment, locks, traps, DCs, shown state, session overrides, player-map obstacle badges, the Map Lab inspector, or removal of map knowledge and player preview | A stage ships, or its scope or settled decisions change |
| [Map Lab component refactor — smaller responsibility-oriented component files](plans/active/maplab-component-refactor/maplab-component-refactor.md) | Plan | Working | Planned — waiting for Map Obstacle State to finish before changing shared Map Lab components. | Splitting or reorganizing oversized Map Lab frontend components without changing the viewer or editor experience. | A stage ships, or its scope or settled decisions change |
| [Map Lab test suite refactor — smaller, behavior-oriented test files](plans/active/maplab-test-suite-refactor/maplab-test-suite-refactor.md) | Plan | Working | Planned — waiting for Map Obstacle State to finish before changing shared Map Lab tests. | Splitting or reorganizing oversized Map Lab frontend tests without changing product behavior or reducing regression coverage. | A stage ships, or its scope or settled decisions change |
