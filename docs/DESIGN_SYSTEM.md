...
- **Kid map contrast** — the kid map (`/play/map`) does not inherit the DM dark surface stack. Its
  drawing colors come from the `--kid-map-*` aliases in `theme.css`, which invert the figure/ground
  relationship: rooms are painted in `--md-on-surface` so a room reads at 11.57:1 against its floor
  plate, where the inherited DM tokens measured 1.08:1. Documented exception: door vs. wall is
  1.83:1 by luminance, separated instead by warm hue and stroke weight (door 12, wall 6).
