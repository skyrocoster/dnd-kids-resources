...
| The kid-facing app at `/play`, the curtain, fog, or what the party has earned | [Player App](areas/player-app.md) — [Player App Skeleton](plans/active/player-app-skeleton.md) next | `UX_PATTERNS.md`, `DESIGN_SYSTEM.md`, `ARCHITECTURE.md`, `DATA_MODEL.md`, `TESTING.md`, then the plan |
| Players, recovery profiles, or player spell/weapon rosters | [Players](areas/players.md) — [Player Spellbook Recovery](plans/active/player-spellbook-recovery.md) (Stage 1-3 shipped; Stage 4 next) | `DATA_MODEL.md`, `API_REFERENCE.md`, `UX_PATTERNS.md`, `TESTING.md`, then the plan |
...
| Shared UI, tokens, icons, or accessibility | [Visual Design](areas/visual-design.md) — plan archived | `DESIGN_SYSTEM.md`, `ARCHITECTURE.md`, `TESTING.md` |
...
| [plans/active/player-spellbook-recovery.md](plans/active/player-spellbook-recovery.md) | Plan | Working | Active (Stage 1-3 shipped) | Player recovery data, spell-first reference, or character assignments | A stage ships, or scope changes |
...
| [plans/active/player-app-skeleton.md](plans/active/player-app-skeleton.md) | Plan | Working | Active (next up) | The `/play` shell, the import rule, the fog and curtain seams, or the kid map | A stage ships, or its scope or settled decisions change |
