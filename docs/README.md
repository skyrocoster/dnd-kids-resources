...
| Room label placement or sizing, in either app | [Players](areas/players.md) — the shared `roomLabelAnchor` shipped in [Kid Map Legibility](plans/done/kid-map-legibility/kid-map-legibility.md) Stage 2 (archived) and survives; label fading is re-planned in [Kid Map Viewer](plans/active/kid-map-viewer/kid-map-viewer.md) Stage 3 | `UX_PATTERNS.md`, `DESIGN_SYSTEM.md`, `TESTING.md`, then the active plan |
...
| The kid-facing app at `/play`, the curtain, fog, what the party has earned, or how the kid map draws rooms, doors, stairs, markers and names | [Players](areas/players.md) — [Player Map Knowledge](plans/active/player-map-knowledge/player-map-knowledge.md) next | `UX_PATTERNS.md`, `DESIGN_SYSTEM.md`, `ARCHITECTURE.md`, `DATA_MODEL.md`, `TESTING.md`, then the plan |
| The kid map's colour language, the four families, or the kid palette solver | [Players](areas/players.md) — [Kid Map Viewer](plans/active/kid-map-viewer/kid-map-viewer.md) Stage 5 | `DESIGN_SYSTEM.md`, `UX_PATTERNS.md`, then the plan; `scripts/generate-md3-tokens.mjs` stays DM-side and is not extended |
