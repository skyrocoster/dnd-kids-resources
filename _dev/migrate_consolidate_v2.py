...
def migrate():
    ...
    try:
        ...
        if 'title' not in spells_cols:
            cursor.execute("ALTER TABLE spells ADD COLUMN title TEXT NOT NULL DEFAULT ''")
        if 'icon' not in spells_cols:
            cursor.execute("ALTER TABLE spells ADD COLUMN icon TEXT NOT NULL DEFAULT '✨'")
        if 'level' not in spells_cols:
            cursor.execute("ALTER TABLE spells ADD COLUMN level TEXT NOT NULL DEFAULT 'cantrip'")
        if 'explanation' not in spells_cols:
            cursor.execute("ALTER TABLE spells ADD COLUMN explanation TEXT DEFAULT ''")
